...
// Browser globals
a.JSONEditor=b(a.jQuery)}(this,function(a){
// 2020/4/2 添加  ↑
// intro.js ↑
...
var b;!function(){var a=!1,c=/xyz/.test(function(){window.postMessage("xyz")})?/\b_super\b/:/.*/;
// The base Class implementation (does nothing)
// Create a new Class that inherits from this class
return b=function(){},b.extend=function d(b){
...
g[h]="function"==typeof b[h]&&"function"==typeof f[h]&&c.test(b[h])?function(a,b){return function(){var c=this._super;
// Add a new ._super() method that is the same method
// but on the super-class
this._super=f[a];
// The method only need to be bound temporarily, so we
// remove it when we're done executing
var d=b.apply(this,arguments);return this._super=c,d}}(h,b[h]):b[h];
// Populate our constructed prototype object
// Enforce the constructor to be what we expect
// And make this class extendable
return e.prototype=g,e.prototype.constructor=e,e.extend=d,e},b}(),
...
// - DOM nodes
// - window
return!("object"!=typeof a||a.nodeType||null!==a&&a===a.window)&&!(a.constructor&&!Object.prototype.hasOwnProperty.call(a.constructor.prototype,"isPrototypeOf"))},d=function(a){var b,e,f;for(e=1;e<arguments.length;e++){b=arguments[e];for(f in b)b.hasOwnProperty(f)&&(b[f]&&c(b[f])?(a.hasOwnProperty(f)||(a[f]={}),d(a[f],b[f])):a[f]=b[f])}return a},e=function(a,b){if(a&&"object"==typeof a){var c;if(Array.isArray(a)||"number"==typeof a.length&&a.length>0&&a.length-1 in a){for(c=0;c<a.length;c++)if(b(c,a[c])===!1)return}else if(Object.keys){var d=Object.keys(a);for(c=0;c<d.length;c++)if(b(d[c],a[d[c]])===!1)return}else for(c in a)if(a.hasOwnProperty(c)&&b(c,a[c])===!1)return}},f=function(a,b){var c=document.createEvent("HTMLEvents");c.initEvent(b,!0,!0),a.dispatchEvent(c)},g=function(a,b){if(!(a instanceof Element))throw new Error("element should be an instance of Element");b=d({},g.defaults.options,b||{}),this.element=a,this.options=b,this.init()};g.prototype={
// necessary since we remove the ctor property by doing a literal assignment. Without this
// the $isplainobject function will think that this is a plain object.
constructor:g,init:function(){var a=this;this.ready=!1,this.copyClipboard=null;var b=g.defaults.themes[this.options.theme||g.defaults.theme];if(!b)throw"Unknown theme "+(this.options.theme||g.defaults.theme);this.schema=this.options.schema,this.theme=new b,this.template=this.options.template,this.refs=this.options.refs||{},this.uuid=0,this.__data={};var c=g.defaults.iconlibs[this.options.iconlib||g.defaults.iconlib];c&&(this.iconlib=new c),this.root_container=this.theme.getContainer(),this.element.appendChild(this.root_container),this.translate=this.options.translate||g.defaults.translate,
// Fetch all external refs via ajax
this._loadExternalRefs(this.schema,function(){a._getDefinitions(a.schema);
// Validator options
var b={};a.options.custom_validators&&(b.custom_validators=a.options.custom_validators),a.validator=new g.Validator(a,null,b);
...
if(a&&b){this.callbacks=this.callbacks||{},this.callbacks[a]=this.callbacks[a]||[];for(var c=[],d=0;d<this.callbacks[a].length;d++)this.callbacks[a][d]!==b&&c.push(this.callbacks[a][d]);this.callbacks[a]=c}else a?(this.callbacks=this.callbacks||{},this.callbacks[a]=[]):this.callbacks={};return this},/*
  ...
  */
trigger:function(){var a=Array.prototype.shift.call(arguments);if(this.callbacks&&this.callbacks[a]&&this.callbacks[a].length)for(var b=this.callbacks[a],c=0;c<b.length;c++){var d=b[c];arguments.length&&d.apply(this,arguments)}return this},setOption:function(a,b){if("show_errors"!==a)throw"Option "+a+" must be set during instantiation and cannot be changed later";return this.options.show_errors=b,this.onChange(),this},getEditorClass:function(a){var b;if(a=this.expandSchema(a),e(g.defaults.resolvers,function(c,d){var e=d(a);if(e&&g.defaults.editors[e])return b=e,!1}),!b)throw"Unknown editor for schema "+JSON.stringify(a);if(!g.defaults.editors[b])throw"Unknown editor "+b;return g.defaults.editors[b]},createEditor:function(a,b){return b=d({},a.options||{},b),new a(b)},/*
  onChange: function() {
  ...
  */
onChange:function(a,b,c){if(this.ready&&!this.firing_change){this.firing_change=!0;var d=this;
// Fire change event
if(window.requestAnimationFrame(function(){d.firing_change=!1,d.ready&&(
// Validate and cache results
d.validation_results=d.validator.validate(d.root.getValue()),"never"!==d.options.show_errors?d.root.showValidationErrors(d.validation_results):d.root.showValidationErrors([]))}),void 0!==a)return this.trigger("change",a,b,c),this}},compileTemplate:function(a,b){b=b||g.defaults.template;var c;
// Specifying a preset engine
if("string"==typeof b){if(!g.defaults.templates[b])throw"Unknown template engine "+b;if(c=g.defaults.templates[b](),!c)throw"Template engine "+b+" missing required library."}else c=b;if(!c)throw"No template engine set";if(!c.compile)throw"Invalid template engine set";return c.compile(a)},_data:function(a,b,c){
// Setting data
...
if(!b)return this.watchlist[a]=null,this;for(var c=[],d=0;d<this.watchlist[a].length;d++)this.watchlist[a][d]!==b&&c.push(this.watchlist[a][d]);return this.watchlist[a]=c.length?c:null,this},notifyWatchers:function(a){if(!this.watchlist||!this.watchlist[a])return this;for(var b=0;b<this.watchlist[a].length;b++)this.watchlist[a][b]()},isEnabled:function(){return!this.root||this.root.isEnabled()},enable:function(){this.root.enable()},disable:function(){this.root.disable()},_getDefinitions:function(a,b){if(b=b||"#/definitions/",a.definitions)for(var c in a.definitions)a.definitions.hasOwnProperty(c)&&(this.refs[b+c]=a.definitions[c],a.definitions[c].definitions&&this._getDefinitions(a.definitions[c],b+c+"/definitions/"))},_getExternalRefs:function(a){var b={},c=function(a){for(var c in a)a.hasOwnProperty(c)&&(b[c]=!0)};a.$ref&&"object"!=typeof a.$ref&&"#"!==a.$ref.substr(0,1)&&!this.refs[a.$ref]&&(b[a.$ref]=!0);for(var d in a)if(a.hasOwnProperty(d))if(a[d]&&"object"==typeof a[d]&&Array.isArray(a[d]))for(var e=0;e<a[d].length;e++)a[d][e]&&"object"==typeof a[d][e]&&c(this._getExternalRefs(a[d][e]));else a[d]&&"object"==typeof a[d]&&c(this._getExternalRefs(a[d]));return b},_loadExternalRefs:function(a,b){var c=this,d=this._getExternalRefs(a),f=0,g=0,h=!1;e(d,function(a){if(!c.refs[a]){if(!c.options.ajax)throw"Must set ajax option to true to load external ref "+a;c.refs[a]="loading",g++;var d=a;c.options.ajaxBase&&c.options.ajaxBase!=a.substr(0,c.options.ajaxBase.length)&&"http"!=a.substr(0,4)&&(d=c.options.ajaxBase+a);var e=new XMLHttpRequest;e.open("GET",d,!0),c.options.ajaxCredentials&&(e.withCredentials=c.options.ajaxCredentials),e.onreadystatechange=function(){if(4==e.readyState){
// Request succeeded
if(200!==e.status)throw window.console.log(e),"Failed to fetch ref via ajax- "+a;var i;try{i=JSON.parse(e.responseText)}catch(j){throw window.console.log(j),"Failed to parse external ref "+d}if(!i||"object"!=typeof i)throw"External ref does not contain a valid schema - "+d;c.refs[a]=i,c._loadExternalRefs(i,function(){f++,f>=g&&!h&&(h=!0,b())})}},e.send()}}),g||b()},expandRefs:function(a){for(a=d({},a);a.$ref;){var b=a.$ref;delete a.$ref,this.refs[b]||(b=decodeURIComponent(b)),a=this.extendSchemas(a,this.refs[b])}return a},expandSchema:function(a){var b,c=this,f=d({},a);
// allOf schemas should be merged into the parent
if(
// Version 3 `type`
"object"==typeof a.type&&(
// Array of types
Array.isArray(a.type)?e(a.type,function(b,d){
// Schema
"object"==typeof d&&(a.type[b]=c.expandSchema(d))}):a.type=c.expandSchema(a.type)),
// Version 3 `disallow`
"object"==typeof a.disallow&&(
// Array of types
Array.isArray(a.disallow)?e(a.disallow,function(b,d){
// Schema
"object"==typeof d&&(a.disallow[b]=c.expandSchema(d))}):a.disallow=c.expandSchema(a.disallow)),
// Version 4 `anyOf`
a.anyOf&&e(a.anyOf,function(b,d){a.anyOf[b]=c.expandSchema(d)}),
// Version 4 `dependencies` (schema dependencies)
a.dependencies&&e(a.dependencies,function(b,d){"object"!=typeof d||Array.isArray(d)||(a.dependencies[b]=c.expandSchema(d))}),
// Version 4 `not`
...
if(a.oneOf){var g=d({},f);for(delete g.oneOf,b=0;b<a.oneOf.length;b++)f.oneOf[b]=this.extendSchemas(this.expandSchema(a.oneOf[b]),g)}return this.expandRefs(f)},extendSchemas:function(a,b){a=d({},a),b=d({},b);var c=this,f={};
// Properties in obj2 that aren't in obj1
return e(a,function(a,d){
// If this key is also defined in obj2, merge them
"undefined"!=typeof b[a]?
// Required and defaultProperties arrays should be unioned together
"required"!==a&&"defaultProperties"!==a||"object"!=typeof d||!Array.isArray(d)?"type"!==a||"string"!=typeof d&&!Array.isArray(d)?"object"==typeof d&&Array.isArray(d)?f[a]=d.filter(function(c){return b[a].indexOf(c)!==-1}):"object"==typeof d&&null!==d?f[a]=c.extendSchemas(d,b[a]):f[a]=d:(
// Make sure we're dealing with arrays
"string"==typeof d&&(d=[d]),"string"==typeof b.type&&(b.type=[b.type]),
// If type is only defined in the first schema, keep it
b.type&&b.type.length?f.type=d.filter(function(a){return b.type.indexOf(a)!==-1}):f.type=d,
// If there's only 1 type and it's a primitive, use a string instead of array
1===f.type.length&&"string"==typeof f.type[0]?f.type=f.type[0]:0===f.type.length&&delete f.type):
// Union arrays and unique
f[a]=d.concat(b[a]).reduce(function(a,b){return a.indexOf(b)<0&&a.push(b),a},[]):f[a]=d}),e(b,function(b,c){"undefined"==typeof a[b]&&(f[b]=c)}),f},setCopyClipboardContents:function(a){this.copyClipboard=a},getCopyClipboardContents:function(){return this.copyClipboard}},g.defaults={themes:{},templates:{},iconlibs:{},editors:{},languages:{},resolvers:[],custom_validators:[]},
// core.js ↑
g.Validator=b.extend({init:function(a,b,c){this.jsoneditor=a,this.schema=b||this.jsoneditor.schema,this.options=c||{},this.translate=this.jsoneditor.translate||g.defaults.translate},validate:function(a){return this._validateSchema(this.schema,a)},_validateSchema:function(a,b,c){var f,h,i,j=this,k=[],l=JSON.stringify(b);/*
     * Type Agnostic Validation
     */
// Version 3 `required` and `required_by_default`
if(c=c||"root",
// Work on a copy of the schema
a=d({},this.jsoneditor.expandRefs(a)),"undefined"==typeof b)return("undefined"!=typeof a.required&&a.required===!0||"undefined"==typeof a.required&&this.jsoneditor.options.required_by_default===!0)&&k.push({path:c,property:"required",message:this.translate("error_notset")}),k;
// `enum`
if(a["enum"]){for(f=!1,h=0;h<a["enum"].length;h++)l===JSON.stringify(a["enum"][h])&&(f=!0);f||k.push({path:c,property:"enum",message:this.translate("error_enum")})}
// `extends` (version 3)
if(a["extends"])for(h=0;h<a["extends"].length;h++)k=k.concat(this._validateSchema(a["extends"][h],b,c));
// `allOf`
if(a.allOf)for(h=0;h<a.allOf.length;h++)k=k.concat(this._validateSchema(a.allOf[h],b,c));
// `anyOf`
if(a.anyOf){for(f=!1,h=0;h<a.anyOf.length;h++)if(!this._validateSchema(a.anyOf[h],b,c).length){f=!0;break}f||k.push({path:c,property:"anyOf",message:this.translate("error_anyOf")})}
// `oneOf`
if(a.oneOf){f=0;var m=[];for(h=0;h<a.oneOf.length;h++){
// Set the error paths to be path.oneOf[i].rest.of.path
var n=this._validateSchema(a.oneOf[h],b,c);for(n.length||f++,i=0;i<n.length;i++)n[i].path=c+".oneOf["+h+"]"+n[i].path.substr(c.length);m=m.concat(n)}1!==f&&(k.push({path:c,property:"oneOf",message:this.translate("error_oneOf",[f])}),k=k.concat(m))}
// `type` (both Version 3 and Version 4 support)
if(
// `not`
a.not&&(this._validateSchema(a.not,b,c).length||k.push({path:c,property:"not",message:this.translate("error_not")})),a.type)
// Union type
if(Array.isArray(a.type)){for(f=!1,h=0;h<a.type.length;h++)if(this._checkType(a.type[h],b)){f=!0;break}f||k.push({path:c,property:"type",message:this.translate("error_type_union")})}else this._checkType(a.type,b)||k.push({path:c,property:"type",message:this.translate("error_type",[a.type])});
// `disallow` (version 3)
if(a.disallow)
// Union type
if(Array.isArray(a.disallow)){for(f=!0,h=0;h<a.disallow.length;h++)if(this._checkType(a.disallow[h],b)){f=!1;break}f||k.push({path:c,property:"disallow",message:this.translate("error_disallow_union")})}else this._checkType(a.disallow,b)&&k.push({path:c,property:"disallow",message:this.translate("error_disallow",[a.disallow])});/*
...
f=b/o===Math.floor(b/o),
// Use math.js is available
window.math?f=window.math.mod(window.math.bignumber(b),window.math.bignumber(o)).equals(0):window.Decimal&&(f=new window.Decimal(b).mod(new window.Decimal(o)).equals(0)),f||k.push({path:c,property:a.multipleOf?"multipleOf":"divisibleBy",message:this.translate("error_multipleOf",[o])})}
// `maximum`
a.hasOwnProperty("maximum")&&(
// Vanilla JS, prone to floating point rounding errors (e.g. .999999999999999 == 1)
f=a.exclusiveMaximum?b<a.maximum:b<=a.maximum,
// Use math.js is available
window.math?f=window.math[a.exclusiveMaximum?"smaller":"smallerEq"](window.math.bignumber(b),window.math.bignumber(a.maximum)):window.Decimal&&(f=new window.Decimal(b)[a.exclusiveMaximum?"lt":"lte"](new window.Decimal(a.maximum))),f||k.push({path:c,property:"maximum",message:this.translate(a.exclusiveMaximum?"error_maximum_excl":"error_maximum_incl",[a.maximum])})),
// `minimum`
a.hasOwnProperty("minimum")&&(
// Vanilla JS, prone to floating point rounding errors (e.g. .999999999999999 == 1)
f=a.exclusiveMinimum?b>a.minimum:b>=a.minimum,
// Use math.js is available
window.math?f=window.math[a.exclusiveMinimum?"larger":"largerEq"](window.math.bignumber(b),window.math.bignumber(a.minimum)):window.Decimal&&(f=new window.Decimal(b)[a.exclusiveMinimum?"gt":"gte"](new window.Decimal(a.minimum))),f||k.push({path:c,property:"minimum",message:this.translate(a.exclusiveMinimum?"error_minimum_excl":"error_minimum_incl",[a.minimum])}))}else if("string"==typeof b)
// `maxLength`
...
// Validate against it
if(a.items[h])k=k.concat(this._validateSchema(a.items[h],b[h],c+"."+h));else{if(a.additionalItems===!0)break;if(!a.additionalItems){if(a.additionalItems===!1){k.push({path:c,property:"additionalItems",message:this.translate("error_additionalItems")});break}break}k=k.concat(this._validateSchema(a.additionalItems,b[h],c+"."+h))}else
// Each item in the array must validate against the schema
for(h=0;h<b.length;h++)k=k.concat(this._validateSchema(a.items,b[h],c+"."+h));
// `uniqueItems`
if(
// `maxItems`
a.maxItems&&b.length>a.maxItems&&k.push({path:c,property:"maxItems",message:this.translate("error_maxItems",[a.maxItems])}),
// `minItems`
a.minItems&&b.length<a.minItems&&k.push({path:c,property:"minItems",message:this.translate("error_minItems",[a.minItems])}),a.uniqueItems){var p={};for(h=0;h<b.length;h++){if(f=JSON.stringify(b[h]),p[f]){k.push({path:c,property:"uniqueItems",message:this.translate("error_uniqueItems")});break}p[f]=!0}}}else if("object"==typeof b&&null!==b){
// `maxProperties`
if(a.maxProperties){f=0;for(h in b)b.hasOwnProperty(h)&&f++;f>a.maxProperties&&k.push({path:c,property:"maxProperties",message:this.translate("error_maxProperties",[a.maxProperties])})}
// `minProperties`
if(a.minProperties){f=0;for(h in b)b.hasOwnProperty(h)&&f++;f<a.minProperties&&k.push({path:c,property:"minProperties",message:this.translate("error_minProperties",[a.minProperties])})}
// Version 4 `required`
if("undefined"!=typeof a.required&&Array.isArray(a.required))for(h=0;h<a.required.length;h++)"undefined"==typeof b[a.required[h]]&&k.push({path:c,property:"required",message:this.translate("error_required",[a.required[h]])});
// `properties`
var q={};if(a.properties)if("undefined"!=typeof a.required&&Array.isArray(a.required))for(h=0;h<a.required.length;h++){var r=a.required[h];q[r]=!0,k=k.concat(this._validateSchema(a.properties[r],b[r],c+"."+r))}else for(h in a.properties)a.properties.hasOwnProperty(h)&&(q[h]=!0,k=k.concat(this._validateSchema(a.properties[h],b[h],c+"."+h)));
// `patternProperties`
if(a.patternProperties)for(h in a.patternProperties)if(a.patternProperties.hasOwnProperty(h)){var s=new RegExp(h);
// Check which properties match
for(i in b)b.hasOwnProperty(i)&&s.test(i)&&(q[i]=!0,k=k.concat(this._validateSchema(a.patternProperties[h],b[i],c+"."+i)))}
...
if(!a.additionalProperties){k.push({path:c,property:"additionalProperties",message:this.translate("error_additional_properties",[h])});break}if(a.additionalProperties===!0)break;k=k.concat(this._validateSchema(a.additionalProperties,b[h],c+"."+h))}
// `dependencies`
if(a.dependencies)for(h in a.dependencies)if(a.dependencies.hasOwnProperty(h)&&"undefined"!=typeof b[h])
// Property dependency
if(Array.isArray(a.dependencies[h]))for(i=0;i<a.dependencies[h].length;i++)"undefined"==typeof b[a.dependencies[h][i]]&&k.push({path:c,property:"dependencies",message:this.translate("error_dependency",[a.dependencies[h][i]])});else k=k.concat(this._validateSchema(a.dependencies[h],b,c))}
// Custom type validation (global)
// Custom type validation (instance specific)
return e(g.defaults.custom_validators,function(d,e){k=k.concat(e.call(j,a,b,c))}),this.options.custom_validators&&e(this.options.custom_validators,function(d,e){k=k.concat(e.call(j,a,b,c))}),k},_checkType:function(a,b){
// Simple types
// Simple types
...
if(this.header.children.length){for(var a=0;a<this.header.childNodes.length;a++)if(3===this.header.childNodes[a].nodeType){this.header.childNodes[a].nodeValue=this.getHeaderText();break}}else this.header.textContent=this.getHeaderText()},getHeaderText:function(a){return this.header_text?this.header_text:a?this.schema.title:this.getTitle()},onWatchedFieldChange:function(){var a;if(this.header_template){a=d(this.getWatchedFieldValues(),{key:this.key,i:this.key,i0:1*this.key,i1:1*this.key+1,title:this.getTitle()});var b=this.header_template(a);b!==this.header_text&&(this.header_text=b,this.updateHeaderText(),this.notify())}if(this.link_watchers.length){a=this.getWatchedFieldValues();for(var c=0;c<this.link_watchers.length;c++)this.link_watchers[c](a)}},setValue:function(a){this.value=a},getValue:function(){if(this.dependenciesFulfilled)return this.value},refreshValue:function(){},getChildEditors:function(){return!1},destroy:function(){var a=this;this.unregister(this),e(this.watched,function(b,c){a.jsoneditor.unwatch(c,a.watch_listener)}),this.watched=null,this.watched_values=null,this.watch_listener=null,this.header_text=null,this.header_template=null,this.value=null,this.container&&this.container.parentNode&&this.container.parentNode.removeChild(this.container),this.container=null,this.jsoneditor=null,this.schema=null,this.path=null,this.key=null,this.parent=null},getDefault:function(){if(!this.schema.disable){if(this.schema["default"])return this.schema["default"];if(this.schema["enum"])return this.schema["enum"][0];if("minicolor"==this.schema.format)return g.plugins.minicolor.defaultValue}var a=this.schema.type||this.schema.oneOf;if(a&&Array.isArray(a)&&(a=a[0]),a&&"object"==typeof a&&(a=a.type),a&&Array.isArray(a)&&(a=a[0]),"string"==typeof a){if("number"===a)return 0;if("boolean"===a)return!1;if("integer"===a)return 0;if("string"===a)return"";if("object"===a)return{};if("array"===a)return[]}return null},getTitle:function(){return this.schema.title||this.key},enable:function(){this.disabled=!1},disable:function(){this.disabled=!0},isEnabled:function(){return!this.disabled},isRequired:function(){return"boolean"==typeof this.schema.required?this.schema.required:this.parent&&this.parent.schema&&Array.isArray(this.parent.schema.required)?this.parent.schema.required.indexOf(this.key)>-1:!!this.jsoneditor.options.required_by_default},getDisplayText:function(a){var b=[],c={};
// Determine how many times each attribute name is used.
// This helps us pick the most distinct display text for the schemas.
e(a,function(a,b){b.title&&(c[b.title]=c[b.title]||0,c[b.title]++),b.description&&(c[b.description]=c[b.description]||0,c[b.description]++),b.format&&(c[b.format]=c[b.format]||0,c[b.format]++),b.type&&(c[b.type]=c[b.type]||0,c[b.type]++)}),
// Determine display text for each element of the array
e(a,function(a,d){var e;
...
var d={};return e(b,function(a,e){d[e]=d[e]||0,d[e]++,c[e]>1&&(b[a]=e+" "+d[e])}),b},getOption:function(a){try{throw"getOption is deprecated"}catch(b){window.console.error(b)}return this.options[a]},showValidationErrors:function(a){}}),g.defaults.editors["null"]=g.AbstractEditor.extend({getValue:function(){if(this.dependenciesFulfilled)return null},setValue:function(){this.onChange()},getNumColumns:function(){return 2}}),
// string.js ↓
...
// Input too short
if(a.offsetHeight<a.scrollHeight)for(b=0;a.offsetHeight<a.scrollHeight+3&&!(b>100);)b++,c++,a.style.height=c+"px";else{for(b=0;a.offsetHeight>=a.scrollHeight+3&&!(b>100);)b++,c--,a.style.height=c+"px";a.style.height=c+1+"px"}}},this.input.addEventListener("keyup",function(b){a.adjust_height(this)}),this.input.addEventListener("change",function(b){a.adjust_height(this)}),this.adjust_height()),this.format&&this.input.setAttribute("data-schemaformat",this.format),
// this.control = this.theme.getFormControl(this.label, this.input, this.description, this.infoButton);
this.control=this.theme.getFormControlB3(this.label,this.input,this.description,this.infoButton),this.container.appendChild(this.control),
...
//监听checkbox
this.checkListener()},initstatus:function(a){//初始化状态 是否禁用
this.schema.disable===!1?(this.input.disabled=!0,this.theme.disableLabel(a),this.control.firstElementChild.checked=!1):this.control.firstElementChild.checked=!0},enable:function(){this.always_disabled||(this.input.disabled=!1,
// TODO: WYSIWYG and Markdown editors
this.label&&this.theme.enableLabel(this.label),this.value=this.input.value),this.refreshValue(),this.onChange(!0),this._super()},disable:function(a){a&&(this.always_disabled=!0),this.input.disabled=!0,
// TODO: WYSIWYG and Markdown editors
this.label&&this.theme.disableLabel(this.label),this.value=null,
//此处为了禁用后input中仍显示之前的值，所以不进行refresh
this.onChange(!0),this._super()},afterInputReady:function(){var a,b=this;
// Code editor
if(this.source_code)
// WYSIWYG html and bbcode editor
if(this.options.wysiwyg&&["html","bbcode"].indexOf(this.input_type)>=0&&window.jQuery&&window.jQuery.fn&&window.jQuery.fn.sceditor)a=d({},{plugins:"html"===b.input_type?"xhtml":"bbcode",emoticonsEnabled:!1,width:"100%",height:300},g.plugins.sceditor,b.options.sceditor_options||{}),window.jQuery(b.input).sceditor(a),b.sceditor_instance=window.jQuery(b.input).sceditor("instance"),b.sceditor_instance.blur(function(){
// Get editor's value
...
b.input.value=a.html(),b.value=b.input.value,b.is_dirty=!0,b.onChange(!0)});else if("markdown"===this.input_type&&window.SimpleMDE)a=d({},g.plugins.SimpleMDE,{element:this.input}),this.SimpleMDE=new window.SimpleMDE(a),this.SimpleMDE.codemirror.on("change",function(){b.value=b.SimpleMDE.value(),b.is_dirty=!0,b.onChange(!0)});else if(window.ace){var c=this.input_type;
// aliases for c/cpp
...
// The theme
g.plugins.ace.theme&&this.ace_editor.setTheme("ace/theme/"+g.plugins.ace.theme),
// The mode
this.ace_editor.getSession().setMode("ace/mode/"+c),
...
//checkbox点击监听
checkListener:function(){var a=this,b=a.control.firstElementChild;b.addEventListener("change",function(b){b.preventDefault(),b.stopPropagation(),this.checked?a.enable():a.disable()})}}),
// string.js ↑
/**
 * Created by Mehmet Baker on 12.04.2017
 */
g.defaults.editors.hidden=g.AbstractEditor.extend({register:function(){this._super(),this.input&&this.input.setAttribute("name",this.formname)},unregister:function(){this._super(),this.input&&this.input.removeAttribute("name")},setValue:function(a,b,c){if((!this.template||c)&&(null===a||"undefined"==typeof a?a="":"object"==typeof a?a=JSON.stringify(a):"string"!=typeof a&&(a=""+a),a!==this.serialized)){
// Sanitize value before setting it
...
// Compile and store the template
this.schema.template?(this.template=this.jsoneditor.compileTemplate(this.schema.template,this.template_engine),this.refreshValue()):this.refreshValue()}}),g.defaults.editors.number=g.defaults.editors.string.extend({build:function(){if(this._super(),"undefined"!=typeof this.schema.minimum){var a=this.schema.minimum;"undefined"!=typeof this.schema.exclusiveMinimum&&(a+=1),this.input.setAttribute("min",a)}if("undefined"!=typeof this.schema.maximum){var b=this.schema.maximum;"undefined"!=typeof this.schema.exclusiveMaximum&&(b-=1),this.input.setAttribute("max",b)}},sanitize:function(a){return(a+"").replace(/[^0-9\.\-eE]/g,"")},getNumColumns:function(){return 2},getValue:function(){if(this.dependenciesFulfilled)return""===this.value?void 0:1*this.value}}),g.defaults.editors.integer=g.defaults.editors.number.extend({sanitize:function(a){return a+="",a.replace(/[^0-9\-]/g,"")},getNumColumns:function(){return 2}}),
// rating.js ↓
g.defaults.editors.rating=g.defaults.editors.integer.extend({build:function(){var a,b=this;this.options.compact||(this.header=this.label=this.theme.getFormInputLabel(this.getTitle())),this.schema.description&&(this.description=this.theme.getFormInputDescription(this.schema.description));
// Dynamically add the required CSS the first time this editor is used
var c="json-editor-style-rating",d=document.getElementById(c);if(!d){var f=document.createElement("style");f.id=c,f.type="text/css",f.innerHTML="      .rating-container {        display: inline-block;        clear: both;      }            .rating {        float:left;      }            /* :not(:checked) is a filter, so that browsers that don’t support :checked don’t         follow these rules. Every browser that supports :checked also supports :not(), so         it doesn’t make the test unnecessarily selective */      .rating:not(:checked) > input {        position:absolute;        top:-9999px;        clip:rect(0,0,0,0);      }            .rating:not(:checked) > label {        float:right;        width:1em;        padding:0 .1em;        overflow:hidden;        white-space:nowrap;        cursor:pointer;        color:#ddd;      }            .rating:not(:checked) > label:before {        content: '★ ';      }            .rating > input:checked ~ label {        color: #FFB200;      }            .rating:not([readOnly]):not(:checked) > label:hover,      .rating:not([readOnly]):not(:checked) > label:hover ~ label {        color: #FFDA00;      }            .rating:not([readOnly]) > input:checked + label:hover,      .rating:not([readOnly]) > input:checked + label:hover ~ label,      .rating:not([readOnly]) > input:checked ~ label:hover,      .rating:not([readOnly]) > input:checked ~ label:hover ~ label,      .rating:not([readOnly]) > label:hover ~ input:checked ~ label {        color: #FF8C0D;      }            .rating:not([readOnly])  > label:active {        position:relative;        top:2px;        left:2px;      }",document.getElementsByTagName("head")[0].appendChild(f)}this.input=this.theme.getFormInputField("hidden"),this.container.appendChild(this.input);var g=document.createElement("div");g.className="rating-container";var h=document.createElement("div");h.setAttribute("name",this.formname),h.className="rating",g.appendChild(h),this.options.compact&&this.container.setAttribute("class",this.container.getAttribute("class")+" compact"),(this.schema.readOnly||this.schema.readonly)&&(this.always_disabled=!0,this.input.disabled=!0);var i=this.schema.maximum?this.schema.maximum:5;for(this.schema.exclusiveMaximum&&i--,this.inputs=[],a=i;a>0;a--){var j=this.formname+a,k=this.theme.getFormInputField("radio");k.setAttribute("id",j),k.setAttribute("value",a),k.setAttribute("name",this.formname),h.appendChild(k),this.inputs.push(k);var l=document.createElement("label");l.setAttribute("for",j),l.appendChild(document.createTextNode(a+(1==a?" star":" stars"))),h.appendChild(l)}(this.schema.readOnly||this.schema.readonly)&&(this.always_disabled=!0,e(this.inputs,function(a,b){h.setAttribute("readOnly","readOnly"),b.disabled=!0})),g.addEventListener("change",function(a){a.preventDefault(),a.stopPropagation(),b.input.value=a.srcElement.value,b.is_dirty=!0,b.refreshValue(),b.watch_listener(),b.jsoneditor.notifyWatchers(b.path),b.parent?b.parent.onChildEditorChange(b):b.jsoneditor.onChange()}),this.control=this.theme.getFormControl(this.label,g,this.description),this.container.appendChild(this.control),this.refreshValue()},setValue:function(a){var b=this.sanitize(a);if(this.value!==b){var c=this;e(this.inputs,function(a,d){if(d.value===b)return d.checked=!0,c.value=b,c.input.value=c.value,c.watch_listener(),c.jsoneditor.notifyWatchers(c.path),!1})}}}),
// rating.js ↑
// colorpicker.js ↓
/**
 * Created by Administrator on 2017/3/3.
 */
g.defaults.editors.colorpicker=g.AbstractEditor.extend({register:function(){this._super(),this.colorpickerHex=document.getElementsByClassName("colorpicker_hex");for(var a=0;a<this.colorpickerHex.length;a++)for(var b=0;b<this.colorpickerHex[a].getElementsByTagName("input").length;b++)this.colorpickerHex[a].getElementsByTagName("input")[b].setAttribute("name",this.formname)},setValue:function(a){this.value=a,this.change()},getValue:function(){return this.value},build:function(){function a(a){d[a].addEventListener("click",function(a){a.preventDefault(),a.stopPropagation(),b.is_dirty=!0,b.refreshValue(),b.onChange(!0)})}var b=this;this.options.compact||(this.header=this.label=this.theme.getFormInputLabel(this.getTitle())),this.schema.description&&(this.description=this.theme.getFormInputDescription(this.schema.description)),this.format=this.schema.format;var c=this.schema.options.colorpicker_option.mid;
//界面绘制
this.draw(),this.options.compact&&(this.container.className+=" compact"),this.control=this.theme.getFormControl(this.label,this.draw(),this.description),this.container.appendChild(this.control),this.setupColorPicker(c);for(var d=document.getElementsByClassName("colorpicker_submit"),e=0;e<d.length;e++)a(e)},draw:function(){var a,b,c,d=this.schema.options.colorpicker_option.mid;return a=document.createElement("div"),b=document.createElement("div"),c=document.createElement("div"),a.className="color-container row",b.className="col-md-2",c.className="col-md-2",d?(b.id="colorSelector_"+d,c.id="colorpickerHolder_"+d):(b.id="colorSelector_256",c.id="colorpickerHolder_256"),a.appendChild(b),a.appendChild(c),a},setupColorPicker:function(b){var c=this;this.value="ff0000";var e={onSubmit:function(a,b,d,e){c.value=b}},f=d({},g.plugins.colorpicker,e);this.schema.options&&this.schema.options.colorpicker_option&&(f=d(f,this.schema.options.colorpicker_option)),b?this.colorPicker=a("#colorpickerHolder_"+b).ColorPicker(f):this.colorPicker=a("#colorpickerHolder_256").ColorPicker(f)}}),
// colorpicker.js ↑
// checkboxradio.js ↓
/**
 ...
 * 第四步：创建扩展editor
 */
g.defaults.editors.checkboxradio=g.AbstractEditor.extend({register:function(){this._super(),this.input&&this.input.setAttribute("name",this.formname)},typecast:function(a){return"boolean"===this.schema.type?!!a:"number"===this.schema.type?1*a:"integer"===this.schema.type?Math.floor(1*a):""+a},setValue:function(b){b=this.typecast(b||"");var c=b,d=this.enum_options[this.enum_values.indexOf(c)];e(this.checkboxradio,function(b,c){c.value==d&&(a(c).attr("checked","checked"),a(c).prev().addClass(" ui-checkboxradio-checked ui-state-active"))}),this.value=c,this.change()},getValue:function(){return this.value},preBuild:function(){var a=this;this.input_type="select",this.enum_options=[],this.enum_values=[],this.enum_display=[];
// Enum options enumerated
if(this.schema.options.custom_option["enum"]){var b=this.schema.options&&this.schema.options.custom_option&&this.schema.options.custom_option.enum_titles||[];e(this.schema.options.custom_option["enum"],function(c,d){a.enum_options[c]=""+d,a.enum_display[c]=""+(b[c]||d),a.enum_values[c]=a.typecast(d)})}},build:function(){var a=this;this.options.compact||(this.header=this.label=this.theme.getFormInputLabel(this.getTitle())),this.schema.description&&(this.description=this.theme.getFormInputDescription(this.schema.description)),this.format=this.schema.format,
//界面绘制
this.input=this.draw(),
//监听并赋值
this.input.addEventListener("change",function(b){b.preventDefault(),b.stopPropagation(),a.value=b.srcElement.value,a.is_dirty=!0,a.refreshValue(),a.onChange(!0)}),this.options.compact&&(this.container.className+=" compact"),this.control=this.theme.getFormControlB3(this.label,this.input,this.description,this),this.container.appendChild(this.control),this.setupCheckboxRadio(this.label),this.checkListener()},draw:function(){//绘制界面
var a,b,c;a=document.createElement("div"),a.className+=" widget";for(var d=this.theme.GenNonDuplicateID(),e=this.schema.options.custom_option,f=0;f<e.enum_title.length;f++){var g=this.theme.GenNonDuplicateID();b=document.createElement("label"),b.setAttribute("for","radio"+g),b.innerHTML=e.enum_title[f],c=document.createElement("input"),c.setAttribute("type","radio"),c.name="radio"+d,c.id="radio"+g,c.value=e["enum"][f],a.appendChild(b),a.appendChild(c)}return a},setupCheckboxRadio:function(b){var c=d({},g.plugins.checkboxradio);this.schema.options&&this.schema.options.custom_option&&(c=d(c,this.schema.options.custom_option)),this.checkboxradio=a(".widget input").checkboxradio(c),this.initstatus(b)},initstatus:function(b){this.schema.disable?(a(this.input).children("input").checkboxradio("disable"),this.theme.disableLabel(b)):"checkbox"==this.control.firstElementChild.type&&(this.control.firstElementChild.checked=!0)},checkListener:function(){//创建监听checkbox
var a=this,b=a.control.firstElementChild;"checkbox"==b.type&&b.addEventListener("change",function(b){b.preventDefault(),b.stopPropagation(),this.checked?a.enable():a.disable()})},enable:function(){a(this.input).children("input").checkboxradio("enable"),this.theme.enableLabel(this.label),this.value=this.input.value,this.refreshValue(),this.onChange(!0),this._super()},disable:function(){a(this.input).children("input").checkboxradio("disable"),this.theme.disableLabel(this.label),this.input.value=this.value,this.value=null,this.is_dirty=!0,this.onChange(!0),this._super()}}),
// checkboxradio.js ↑
// jpicker.js ↓
/**
 ...
 * 创建颜色拾取器编辑器
 */
g.defaults.editors.jpicker=g.AbstractEditor.extend({register:function(){this._super()},typecast:function(a){return"boolean"===this.schema.type?!!a:"number"===this.schema.type?1*a:"integer"===this.schema.type?Math.floor(1*a):""+a},setValue:function(a){this.value=a,this.change()},getValue:function(){return this.value},build:function(){this.options.compact||(this.header=this.label=this.theme.getFormInputLabel(this.getTitle())),this.schema.description&&(this.description=this.theme.getFormInputDescription(this.schema.description)),this.format=this.schema.format;
//界面绘制
var a=this.theme.GenNonDuplicateID();this.input=this.draw(a),this.options.compact&&(this.container.className+=" compact"),this.control=this.theme.getFormControlB3(this.label,this.input,this.description,this),this.container.appendChild(this.control),this.setupJpicker(this.label,a),
//监听并赋值
this.checkListener()},draw:function(a){var b;return b=document.createElement("input"),b.value=this.schema["default"],b.id=a,b.style.display="none",b.className="jpicker",b},setupJpicker:function(b,c){var e=this,f=function(a,b){var c=a.val("all");null===c?e.value="transparent":e.value="#"+c.hex,e.refreshValue(),e.onChange(!0)},h=d({},g.plugins.jpicker);if(this.schema.options&&this.schema.options.custom_option){var i=this.schema.options.custom_option;h=d(h,i)}a.fn.jPicker.defaults.images.clientPath="lib/vbap-jpicker/dist/images/",this.colorPicker=a("#"+c).jPicker(h,f),this.initstatus(b)},initstatus:function(a){this.schema.disable?
// jQuery(this.checkboxradio).checkboxradio("disable");
this.theme.disableLabel(a):"checkbox"==this.control.firstElementChild.type&&(this.control.firstElementChild.checked=!0)},checkListener:function(){var a=this,b=a.control.firstElementChild;"checkbox"==b.type&&b.addEventListener("change",function(b){b.preventDefault(),b.stopPropagation(),this.checked?a.enable():a.disable()})},enable:function(){this.theme.enableLabel(this.label),
// this.value=jQuery.jPicker.List[0].color.active.val('hex');
this.value=this.input.value;
//去掉遮罩层
var a=this.label.nextSibling.nextSibling.firstChild,b=a.getElementsByTagName("div")[0];a.removeChild(b),this.refreshValue(),this.onChange(!0),this._super()},disable:function(){this.theme.disableLabel(this.label),this.input.value=this.value,this.value=null;
//添加遮罩层
var a=this.label.nextSibling.nextSibling.firstChild,b=document.createElement("div");a.appendChild(b),b.style.backgroundColor="#eee",b.style.width="25px",b.style.height="24px",b.style.position="relative",b.style.top="-20px",b.style.zIndex=10,
// spanjp_div.previousElementSibling.style.position='absolute';
// spanjp_div.parentElement.style.position='absolute';
b.style.border="1px solid #ccc",b.style.borderRadius="50%",b.style.cursor="not-allowed",this.is_dirty=!0,
// this.refreshValue();
this.onChange(!0),this._super()}}),
// jpicker.js ↑
// minicolor.js ↓
/**
 * Created by Administrator on 2017/4/27.
 */
g.defaults.editors.minicolor=g.AbstractEditor.extend({register:function(){this._super()},typecast:function(a){return"boolean"===this.schema.type?!!a:"number"===this.schema.type?1*a:"integer"===this.schema.type?Math.floor(1*a):""+a},setValue:function(b){this.value=b,this.input.value=b,a(this.input).minicolors("value",b),this.change()},getValue:function(){return this.value},refreshValue:function(){this.value=this.getValue()},build:function(){this.options.compact||(this.header=this.label=this.theme.getFormInputLabel(this.getTitle())),this.schema.description&&(this.description=this.theme.getFormInputDescription(this.schema.description)),this.format=this.schema.format;var a=this.theme.GenNonDuplicateID();this.input=this.draw(a),this.options.compact&&(this.container.className+=" compact"),"array"===this.parent.schema.type?this.control=this.theme.getFormControlB3Array(this.label,this.input,this.description,this):this.control=this.theme.getFormControlB3(this.label,this.input,this.description,this),this.container.appendChild(this.control),this.setupMinicolor(this.label,a),
//监听并赋值
this.checkListener()},draw:function(a){var b;return b=document.createElement("input"),b.value=this.schema["default"]?this.schema["default"]:"#ff6161",b.id=a,b.type="hidden",b.className="minicolor",b},setupMinicolor:function(b){var c,e=this,f={change:function(a,b){e.value=a,e.refreshValue(),e.onChange(!0)}};if("array"===this.parent.schema.type){var h={colorArray:!0};f=d(f,h),c=d({},g.plugins.minicolor,f)}else c=d({},g.plugins.minicolor,f);this.miniColor=a(this.input).minicolors(c),this.initstatus(b)},initstatus:function(a){this.schema.disable?this.theme.disableLabel(a):"checkbox"==this.control.firstElementChild.type&&(this.control.firstElementChild.checked=!0)},checkListener:function(){var a=this,b=a.control.firstElementChild;"checkbox"==b.type&&b.addEventListener("change",function(b){b.preventDefault(),b.stopPropagation(),this.checked?a.enable():a.disable()})},enable:function(){this.theme.enableLabel(this.label),this.value=this.input.value;
//去掉遮罩层
var a=this.label.nextSibling,b=a.getElementsByClassName("miniCover")[0];a.removeChild(b),this.refreshValue(),this.onChange(!0),this._super()},disable:function(){this.theme.disableLabel(this.label),this.input.value=this.value,this.value=null;
//添加遮罩层
var a=this.label.nextSibling,b=document.createElement("div");a.appendChild(b),b.className="miniCover",b.style.backgroundColor="#eee",b.style.position="absolute",b.style.top="0",b.style.left="0",b.style.right="0",b.style.bottom="0",b.style.zIndex=50,b.style.cursor="not-allowed",this.is_dirty=!0,
// this.refreshValue();
this.onChange(!0),this._super()}}),
// minicolor.js ↑
// acedit.js ↓
 ...
 */
g.defaults.editors.acedit=g.AbstractEditor.extend({register:function(){this._super(),this.input&&this.input.setAttribute("name",this.formname)},setValue:function(a,b,c){if((!this.template||c)&&(null===a||"undefined"==typeof a?a="":"object"==typeof a?a=JSON.stringify(a):"string"!=typeof a&&(a=""+a),a!==this.serialized)){
// Sanitize value before setting it
var d=this.sanitize(a);if(this.input.value!==d){this.input.value=d,
// If using SCEditor, update the WYSIWYG
this.ace_editor&&this.ace_editor.setValue(d);var e=c||this.getValue()!==a;this.refreshValue(),b?this.is_dirty=!1:"change"===this.jsoneditor.options.show_errors&&(this.is_dirty=!0),this.adjust_height&&this.adjust_height(this.input),
// Bubble this setValue to parents if the value changed
this.onChange(e)}}},sanitize:function(a){var b=a.replace(/(\n\s?)|(\s{3,})/g,"");return b},build:function(){var a=this;this.options.compact||(this.header=this.label=this.theme.getFormInputLabel(this.getTitle())),this.schema.description&&(this.description=this.theme.getFormInputDescription(this.schema.description)),this.format=this.schema.format,this.programe_language=this.schema.programe_language,!this.format&&this.schema.media&&this.schema.media.type&&(this.format=this.schema.media.type.replace(/(^(application|text)\/(x-)?(script\.)?)|(-source$)/g,"")),!this.format&&this.options.default_format&&(this.format=this.options.default_format),this.options.format&&(this.format=this.options.format),
//判断需要支持的语言类型
["actionscript","batchfile","bbcode","c","c++","cpp","coffee","csharp","css","dart","django","ejs","erlang","golang","groovy","handlebars","haskell","haxe","html","ini","jade","java","javascript","json","less","lisp","lua","makefile","markdown","matlab","mysql","objectivec","pascal","perl","pgsql","php","python","r","ruby","sass","scala","scss","smarty","sql","stylus","svg","twig","vbscript","xml","yaml"].indexOf(this.programe_language)>=0&&(this.input_type=this.programe_language,this.source_code=!0,this.input=this.theme.getTextareaInput()),this.options.compact&&(this.container.className+=" compact");var b=this.theme.GenNonDuplicateID();this.input=this.draw(b),this.run_btn=document.createElement("input"),this.run_btn.type="button",this.run_btn.value="RUN",this.input.addEventListener("change",function(b){
// Don't allow changing if this field is a template
if(b.preventDefault(),b.stopPropagation(),a.schema.template)return void(this.value=a.value);var c=this.value,d=a.sanitize(c);c!==d&&(this.value=d),a.is_dirty=!0,a.refreshValue(),a.onChange(!0)}),this.format&&this.input.setAttribute("data-schemaformat",this.format),this.control=this.theme.getFormControlB3(this.label,this.input,this.description,this),this.control.appendChild(this.run_btn),this.container.appendChild(this.control),
// Any special formatting that needs to happen after the input is added to the dom
window.requestAnimationFrame(function(){
// Skip in case the input is only a temporary editor,
// otherwise, in the case of an ace_editor creation,
...
//监听checkbox
this.checkListener()},initstatus:function(a){//初始化状态 是否禁用
this.schema.disable?(this.input.disabled=!0,this.theme.disableLabel(a),this.control.firstElementChild.checked=!1):this.control.firstElementChild.checked=!0},enable:function(){this.always_disabled||(this.input.disabled=!1,this.run_btn.disabled=!1,
// TODO: WYSIWYG and Markdown editors
this.label&&this.theme.enableLabel(this.label),this.value=this.input.value);
//去掉遮罩层
var a=this.label.nextSibling,b=a.getElementsByClassName("aceCover")[0];a.removeChild(b),this.refreshValue(),this.onChange(!0),this._super()},disable:function(){this.input.disabled=!0,this.run_btn.disabled=!0,
// TODO: WYSIWYG and Markdown editors
this.label&&this.theme.disableLabel(this.label),this.value=null;
//添加遮罩层
var a=this.label.nextSibling,b=document.createElement("div");a.appendChild(b),b.className="aceCover",b.style.backgroundColor="#777",b.style.opacity="0.7",b.style.position="absolute",b.style.top="0",b.style.left="0",b.style.right="0",b.style.bottom="0",b.style.zIndex=50,b.style.cursor="not-allowed",
//此处为了禁用后input中仍显示之前的值，所以不进行refresh
this.onChange(!0),this._super()},draw:function(){return this.ace_container=document.createElement("div"),this.ace_container.style.width="100%",this.ace_container.style.position="relative",this.ace_container.style.height="100px",this.ace_container},setupAce:function(){var a=this,b=this.input_type;this.ace_editor=window.ace.edit(this.ace_container),this.ace_editor.setValue(this.sanitize(this.getValue())),
// The theme
g.plugins.ace.theme&&this.ace_editor.setTheme("ace/theme/"+g.plugins.ace.theme),
// The mode
this.ace_editor.getSession().setMode("ace/mode/"+b),
// Listen for changes
this.ace_editor.on("change",function(){var b=a.ace_editor.getValue();a.input.value=b.replace(/(\n\s?)|(\s{3,})/g,""),a.refreshValue(),a.is_dirty=!0,a.onChange(!0)}),this.run_btn.addEventListener("click",function(b){b.preventDefault(),b.stopPropagation();var c=a.input.value,d=document.createElement("script");d.innerHTML=c,document.body.appendChild(d)})},onWatchedFieldChange:function(){var a;
// If this editor needs to be rendered by a macro template
this.template&&(a=this.getWatchedFieldValues(),this.setValue(this.template(a),!1,!0)),this._super()},refreshValue:function(){this.value=this.input.value,"string"!=typeof this.value&&(this.value=""),this.serialized=this.value},
//checkbox点击监听
checkListener:function(){var a=this,b=a.control.firstElementChild;b.addEventListener("change",function(b){b.preventDefault(),b.stopPropagation(),this.checked?a.enable():a.disable()})}}),
// acedit.js ↑
// object.js ↓
...
this.property_order=Object.keys(this.editors),this.property_order=this.property_order.sort(function(a,b){var d=c.editors[a].schema.propertyOrder,e=c.editors[b].schema.propertyOrder;return"number"!=typeof d&&(d=1e3),"number"!=typeof e&&(e=1e3),d-e});var d;if("grid"===this.format){var f=[];
// Make almost full rows width 12
// Do this by increasing all editors' sizes proprotionately
// Any left over space goes to the biggest editor
// Don't touch rows with a width of 6 or less
for(e(this.property_order,function(a,b){var d=c.editors[b];if(!d.property_removed){
...
// Layout the form
d=document.createElement("div"),a=0;a<f.length;a++){var i=this.theme.getGridRow();for(d.appendChild(i),b=0;b<f[a].editors.length;b++){var j=f[a].editors[b].key,k=this.editors[j];k.options.hidden?k.container.style.display="none":this.theme.setGridColumnSize(k.container,f[a].editors[b].width),i.appendChild(k.container)}}}else d=document.createElement("div"),e(this.property_order,function(a,b){var e=c.editors[b];if(!e.property_removed){var f=c.theme.getGridRow();d.appendChild(f),e.options.hidden?e.container.style.display="none":c.theme.setGridColumnSize(e.container,12),f.appendChild(e.container)}});for(;this.row_container.firstChild;)this.row_container.removeChild(this.row_container.firstChild);this.row_container.appendChild(d)}},getPropertySchema:function(a){
// Schema declared directly in properties
var b=this.schema.properties[a]||{};b=d({},b);var c=!!this.schema.properties[a];
// Any matching patternProperties should be merged in
if(this.schema.patternProperties)for(var e in this.schema.patternProperties)if(this.schema.patternProperties.hasOwnProperty(e)){var f=new RegExp(e);f.test(a)&&(b.allOf=b.allOf||[],b.allOf.push(this.schema.patternProperties[e]),c=!0)}
// Hasn't matched other rules, use additionalProperties schema
return!c&&this.schema.additionalProperties&&"object"==typeof this.schema.additionalProperties&&(b=d({},this.schema.additionalProperties)),b},preBuild:function(){this._super(),this.editors={},this.cached_editors={};var a=this;
// If the object should be rendered as a table row
if(this.format=this.options.layout||this.options.object_layout||this.schema.format||this.jsoneditor.options.object_layout||"normal",this.schema.properties=this.schema.properties||{},this.minwidth=0,this.maxwidth=0,this.options.table_row)e(this.schema.properties,function(b,c){var d=a.jsoneditor.getEditorClass(c);a.editors[b]=a.jsoneditor.createEditor(d,{jsoneditor:a.jsoneditor,schema:c,path:a.path+"."+b,parent:a,compact:!0,required:!0}),a.editors[b].preBuild();var e=a.editors[b].options.hidden?0:a.editors[b].options.grid_columns||a.editors[b].getNumColumns();a.minwidth+=e,a.maxwidth+=e}),this.no_link_holder=!0;else{if(this.options.table)
// TODO: table display format
throw"Not supported yet";this.schema.defaultProperties||(this.jsoneditor.options.display_required_only||this.options.display_required_only?(this.schema.defaultProperties=[],e(this.schema.properties,function(b,c){a.isRequired({key:b,schema:c})&&a.schema.defaultProperties.push(b)})):a.schema.defaultProperties=Object.keys(a.schema.properties)),
// Increase the grid width to account for padding
a.maxwidth+=1,e(this.schema.defaultProperties,function(b,c){a.addObjectProperty(c,!0),a.editors[c]&&(a.minwidth=Math.max(a.minwidth,a.editors[c].options.grid_columns||a.editors[c].getNumColumns()),a.maxwidth+=a.editors[c].options.grid_columns||a.editors[c].getNumColumns())})}
// Sort editors by propertyOrder
...
this.row_container=this.theme.getGridContainer(),this.editor_holder.appendChild(this.row_container),e(this.editors,function(b,c){var d=a.theme.getGridColumn();a.row_container.appendChild(d),c.setContainer(d),c.build(),c.postBuild()}),
// Control buttons
...
this.options.collapsed&&f(this.toggle_button,"click"),
...
this.options.table_row?(this.editor_holder=this.container,e(this.property_order,function(b,c){a.editor_holder.appendChild(a.editors[c].container)})):(
// Initial layout
this.layoutEditors(),
...
b||(c.refreshValue(),c.layoutEditors())}},onChildEditorChange:function(a){this.refreshValue(),this._super(a)},canHaveAdditionalProperties:function(){return"boolean"==typeof this.schema.additionalProperties?this.schema.additionalProperties:!this.jsoneditor.options.no_additional_properties},destroy:function(){e(this.cached_editors,function(a,b){b.destroy()}),this.editor_holder&&(this.editor_holder.innerHTML=""),this.title&&this.title.parentNode&&this.title.parentNode.removeChild(this.title),this.error_holder&&this.error_holder.parentNode&&this.error_holder.parentNode.removeChild(this.error_holder),this.editors=null,this.cached_editors=null,this.editor_holder&&this.editor_holder.parentNode&&this.editor_holder.parentNode.removeChild(this.editor_holder),this.editor_holder=null,this._super()},getValue:function(){if(this.dependenciesFulfilled){var a=this._super();if(this.jsoneditor.options.remove_empty_properties||this.options.remove_empty_properties)for(var b in a)/*
...
e(this.cached_editors,function(d,e){
...
if(e(a,function(a,e){e.path===b.path?c.push(e):d.push(e)}),this.error_holder)if(c.length){this.error_holder.innerHTML="",this.error_holder.style.display="",e(c,function(a,c){b.error_holder.appendChild(b.theme.getErrorMessage(c.message))})}else this.error_holder.style.display="none";
...
e(this.editors,function(a,b){b.showValidationErrors(d)})}}),
// object.js ↑
// array.js ↓
g.defaults.editors.array=g.AbstractEditor.extend({getDefault:function(){return this.schema["default"]||[]},register:function(){if(this._super(),this.rows)for(var a=0;a<this.rows.length;a++)this.rows[a].register()},unregister:function(){if(this._super(),this.rows)for(var a=0;a<this.rows.length;a++)this.rows[a].unregister()},getNumColumns:function(){var a=this.getItemInfo(0);
// Tabs require extra horizontal space
// Tabs require extra horizontal space
return this.tabs_holder?Math.max(Math.min(12,a.width+2),4):a.width},enable:function(){if(!this.always_disabled){if(this.add_row_button&&(this.add_row_button.disabled=!1),this.remove_all_rows_button&&(this.remove_all_rows_button.disabled=!1),this.delete_last_row_button&&(this.delete_last_row_button.disabled=!1),this.rows)for(var a=0;a<this.rows.length;a++)this.rows[a].enable(),this.rows[a].moveup_button&&(this.rows[a].moveup_button.disabled=!1),this.rows[a].movedown_button&&(this.rows[a].movedown_button.disabled=!1),this.rows[a].delete_button&&(this.rows[a].delete_button.disabled=!1);this._super()}},disable:function(a){if(a&&(this.always_disabled=!0),this.add_row_button&&(this.add_row_button.disabled=!0),this.remove_all_rows_button&&(this.remove_all_rows_button.disabled=!0),this.delete_last_row_button&&(this.delete_last_row_button.disabled=!0),this.rows)for(var b=0;b<this.rows.length;b++)this.rows[b].disable(a),this.rows[b].moveup_button&&(this.rows[b].moveup_button.disabled=!0),this.rows[b].movedown_button&&(this.rows[b].movedown_button.disabled=!0),this.rows[b].delete_button&&(this.rows[b].delete_button.disabled=!0);this._super()},preBuild:function(){this._super(),this.rows=[],this.row_cache=[],this.hide_delete_buttons=this.options.disable_array_delete||this.jsoneditor.options.disable_array_delete,this.hide_delete_all_rows_buttons=this.hide_delete_buttons||this.options.disable_array_delete_all_rows||this.jsoneditor.options.disable_array_delete_all_rows,
// 2020/4/3 修改 ↓
...
// 2020/4/3 修改 ↑
this.hide_move_buttons=this.options.disable_array_reorder||this.jsoneditor.options.disable_array_reorder,this.hide_add_button=this.options.disable_array_add||this.jsoneditor.options.disable_array_add,this.show_copy_button=this.options.enable_array_copy||this.jsoneditor.options.enable_array_copy},build:function(){this.options.compact?(this.panel=this.theme.getIndentedPanel(),this.container.appendChild(this.panel),this.controls=this.theme.getButtonHolder(),this.panel.appendChild(this.controls),this.row_holder=document.createElement("div"),this.panel.appendChild(this.row_holder)):(this.header=document.createElement("span"),this.header.textContent=this.getTitle(),this.title=this.theme.getHeader(this.header),this.container.appendChild(this.title),this.title_controls=this.theme.getHeaderButtonHolder(),this.title.appendChild(this.title_controls),this.schema.description&&(this.description=this.theme.getDescription(this.schema.description),this.container.appendChild(this.description)),this.error_holder=document.createElement("div"),this.container.appendChild(this.error_holder),"tabs"===this.schema.format?(this.controls=this.theme.getHeaderButtonHolder(),this.title.appendChild(this.controls),this.tabs_holder=this.theme.getTabHolder(),this.container.appendChild(this.tabs_holder),this.row_holder=this.theme.getTabContentHolder(this.tabs_holder),this.active_tab=null):"htabs"===this.schema.format?(
// this.theme.getStyles(this.title,this.schema.styles);
this.controls=this.theme.getHeaderButtonHolder(),this.title.appendChild(this.controls),this.htabs_holder=this.theme.getHTabHolder(),this.container.appendChild(this.htabs_holder),this.row_holder=this.theme.getTabContentHolder(this.htabs_holder),this.active_tab=null):"minicolor"===this.schema.items.format?(this.panel=this.theme.getIndentedPanel(),this.container.appendChild(this.panel),this.row_holder=document.createElement("div"),this.panel.appendChild(this.row_holder),this.controls=this.theme.getButtonHolder(),this.panel.appendChild(this.controls)):(this.panel=this.theme.getIndentedPanel(),this.container.appendChild(this.panel),this.row_holder=document.createElement("div"),this.panel.appendChild(this.row_holder),this.controls=this.theme.getButtonHolder(),this.panel.appendChild(this.controls))),this.theme.getStyles(this.title,this.schema.styles),
// Add controls
...
d||this.hide_delete_last_row_buttons?this.delete_last_row_button.style.display="none":(this.delete_last_row_button.style.display="",f=!0)):(d||this.hide_delete_last_row_buttons?this.delete_last_row_button.style.display="none":(this.delete_last_row_button.style.display="",f=!0),d||this.hide_delete_all_rows_buttons?this.remove_all_rows_button.style.display="none":(this.remove_all_rows_button.style.display="",f=!0)):(this.delete_last_row_button.style.display="none",this.remove_all_rows_button.style.display="none"),
// If there are maxItems in the array, hide the add button beneath the rows
this.getMax()&&this.getMax()<=this.rows.length||this.hide_add_button?this.add_row_button.style.display="none":(this.add_row_button.style.display="",f=!0),!this.collapsed&&f?this.controls.style.display="inline-block":this.controls.style.display="none"}},addRow:function(a,b){var c=this,d=this.rows.length;c.rows[d]=this.getElementEditor(d),c.row_cache[d]=c.rows[d],c.tabs_holder?(c.rows[d].tab_text=document.createElement("span"),c.rows[d].tab_text.textContent=c.rows[d].getHeaderText(),c.rows[d].tab=c.theme.getTab(c.rows[d].tab_text),c.rows[d].tab.addEventListener("click",function(a){c.active_tab=c.rows[d].tab,c.refreshTabs(),a.preventDefault(),a.stopPropagation()}),c.theme.addTab(c.tabs_holder,c.rows[d].tab)):c.htabs_holder&&(c.rows[d].tab_text=document.createElement("span"),c.rows[d].tab_text.textContent=c.rows[d].getHeaderText(),c.rows[d].tab=c.theme.getHTab(c.rows[d].tab_text),c.rows[d].tab.addEventListener("click",function(a){c.active_tab=c.rows[d].tab,c.refreshTabs(),a.preventDefault(),a.stopPropagation()}),c.theme.addTab(c.htabs_holder,c.rows[d].tab));var f=c.rows[d].title_controls||c.rows[d].array_controls;
// Buttons to delete row, move row up, and move row down
c.hide_delete_buttons||(c.rows[d].delete_button=this.getButton(c.getItemTitle(),"delete",this.translate("button_delete_row_title",[c.getItemTitle()])),c.rows[d].delete_button.className+=" delete",c.rows[d].delete_button.setAttribute("data-i",d),c.rows[d].delete_button.addEventListener("click",function(a){a.preventDefault(),a.stopPropagation();var b=1*this.getAttribute("data-i"),d=c.getValue(),f=[],g=null;e(d,function(a,d){
// If the one we're deleting is the active tab
// Make the next tab active if there is one
// Note: the next tab is going to be the current tab after deletion
return a===b?void(c.rows[a].tab===c.active_tab&&(c.rows[a+1]?g=c.rows[a].tab:a&&(g=c.rows[a-1].tab))):void f.push(d)}),c.setValue(f),g&&(c.active_tab=g,c.refreshTabs()),c.onChange(!0)}),f&&f.appendChild(c.rows[d].delete_button)),
//Button to copy an array element and add it as last element
c.show_copy_button&&(c.rows[d].copy_button=this.getButton(c.getItemTitle(),"copy","Copy "+c.getItemTitle()),c.rows[d].copy_button.className+=" copy",c.rows[d].copy_button.setAttribute("data-i",d),c.rows[d].copy_button.addEventListener("click",function(a){var b=c.getValue();a.preventDefault(),a.stopPropagation();var d=1*this.getAttribute("data-i");e(b,function(a,c){if(a===d)return void b.push(c)}),c.setValue(b),c.refreshValue(!0),c.onChange(!0)}),f.appendChild(c.rows[d].copy_button)),d&&!c.hide_move_buttons&&(c.rows[d].moveup_button=this.getButton("","moveup",this.translate("button_move_up_title")),c.rows[d].moveup_button.className+=" moveup",c.rows[d].moveup_button.setAttribute("data-i",d),c.rows[d].moveup_button.addEventListener("click",function(a){a.preventDefault(),a.stopPropagation();var b=1*this.getAttribute("data-i");if(!(b<=0)){var d=c.getValue(),e=d[b-1];d[b-1]=d[b],d[b]=e,c.setValue(d),c.active_tab=c.rows[b-1].tab,c.refreshTabs(),c.onChange(!0)}}),f&&f.appendChild(c.rows[d].moveup_button)),c.hide_move_buttons||(c.rows[d].movedown_button=this.getButton("","movedown",this.translate("button_move_down_title")),c.rows[d].movedown_button.className+=" movedown",c.rows[d].movedown_button.setAttribute("data-i",d),c.rows[d].movedown_button.addEventListener("click",function(a){a.preventDefault(),a.stopPropagation();var b=1*this.getAttribute("data-i"),d=c.getValue();if(!(b>=d.length-1)){var e=d[b+1];d[b+1]=d[b],d[b]=e,c.setValue(d),c.active_tab=c.rows[b+1].tab,c.refreshTabs(),c.onChange(!0)}}),f&&f.appendChild(c.rows[d].movedown_button)),a&&c.rows[d].setValue(a,b),c.refreshTabs()},addControls:function(){var a=this;this.collapsed=!1,this.toggle_button=this.getButton("","collapse",this.translate("button_collapse")),this.title_controls.appendChild(this.toggle_button);var b=a.row_holder.style.display,c=a.controls.style.display;this.toggle_button.addEventListener("click",function(d){d.preventDefault(),d.stopPropagation(),a.collapsed?(a.collapsed=!1,a.panel&&(a.panel.style.display=""),a.row_holder.style.display=b,a.tabs_holder&&(a.tabs_holder.style.display=""),a.controls.style.display=c,a.setButtonText(this,"","collapse",a.translate("button_collapse"))):(a.collapsed=!0,a.row_holder.style.display="none",a.tabs_holder&&(a.tabs_holder.style.display="none"),a.controls.style.display="none",a.panel&&(a.panel.style.display="none"),a.setButtonText(this,"","expand",a.translate("button_expand")))}),
// If it should start collapsed
this.options.collapsed&&f(this.toggle_button,"click"),
// Collapse button disabled
this.schema.options&&"undefined"!=typeof this.schema.options.disable_collapse?this.schema.options.disable_collapse&&(this.toggle_button.style.display="none"):this.jsoneditor.options.disable_collapse&&(this.toggle_button.style.display="none"),
...
if(e(a,function(a,e){e.path===b.path?c.push(e):d.push(e)}),this.error_holder)if(c.length){this.error_holder.innerHTML="",this.error_holder.style.display="",e(c,function(a,c){b.error_holder.appendChild(b.theme.getErrorMessage(c.message))})}else this.error_holder.style.display="none";
// Show errors for child editors
e(this.rows,function(a,b){b.showValidationErrors(d)})}}),
// array.js ↑
...
// Add controls
this.addControls()},onChildEditorChange:function(a){this.refreshValue(),this._super()},getItemDefault:function(){return d({},{"default":this.item_default})["default"]},getItemTitle:function(){return this.item_title},getElementEditor:function(a,b){var c=d({},this.schema.items),e=this.jsoneditor.getEditorClass(c,this.jsoneditor),f=this.row_holder.appendChild(this.theme.getTableRow()),g=f;this.item_has_child_editors||(g=this.theme.getTableCell(),f.appendChild(g));var h=this.jsoneditor.createEditor(e,{jsoneditor:this.jsoneditor,schema:c,container:g,path:this.path+"."+a,parent:this,compact:!0,table_row:!0});return h.preBuild(),b||(h.build(),h.postBuild(),h.controls_cell=f.appendChild(this.theme.getTableCell()),h.row=f,h.table_controls=this.theme.getButtonHolder(),h.controls_cell.appendChild(h.table_controls),h.table_controls.style.margin=0,h.table_controls.style.padding=0),h},destroy:function(){this.innerHTML="",this.title&&this.title.parentNode&&this.title.parentNode.removeChild(this.title),this.description&&this.description.parentNode&&this.description.parentNode.removeChild(this.description),this.row_holder&&this.row_holder.parentNode&&this.row_holder.parentNode.removeChild(this.row_holder),this.table&&this.table.parentNode&&this.table.parentNode.removeChild(this.table),this.panel&&this.panel.parentNode&&this.panel.parentNode.removeChild(this.panel),this.rows=this.title=this.description=this.row_holder=this.table=this.panel=null,this._super()},setValue:function(a,b){
// Make sure value has between minItems and maxItems items in it
if(
// Update the array's value, adding/removing rows when necessary
a=a||[],this.schema.minItems)for(;a.length<this.schema.minItems;)a.push(this.getItemDefault());this.schema.maxItems&&a.length>this.schema.maxItems&&(a=a.slice(0,this.schema.maxItems));var c=JSON.stringify(a);if(c!==this.serialized){var d=!1,f=this;e(a,function(a,b){f.rows[a]?
// TODO: don't set the row's value if it hasn't changed
f.rows[a].setValue(b):(f.addRow(b),d=!0)});for(var g=a.length;g<f.rows.length;g++){var h=f.rows[g].container;f.item_has_child_editors||f.rows[g].row.parentNode.removeChild(f.rows[g].row),f.rows[g].destroy(),h.parentNode&&h.parentNode.removeChild(h),f.rows[g]=null,d=!0}f.rows=f.rows.slice(0,a.length),f.refreshValue(),(d||b)&&f.refreshRowButtons(),f.onChange()}},refreshRowButtons:function(){var a=this,b=this.schema.minItems&&this.schema.minItems>=this.rows.length,c=!1;e(this.rows,function(d,e){
// Hide the move down button for the last row
...
e(this.rows,function(a,b){c?b.controls_cell.style.display="":b.controls_cell.style.display="none"}),c?this.controls_header_cell.style.display="":this.controls_header_cell.style.display="none";var d=!1;this.value.length?1===this.value.length?(this.table.style.display="",this.remove_all_rows_button.style.display="none",
// If there are minItems items in the array, or configured to hide the delete_last_row button, hide the delete button beneath the rows
...
// If there are maxItems in the array, hide the add button beneath the rows
this.schema.maxItems&&this.schema.maxItems<=this.rows.length||this.hide_add_button?this.add_row_button.style.display="none":(this.add_row_button.style.display="",d=!0),d?this.controls.style.display="":this.controls.style.display="none"},refreshValue:function(){var a=this;this.value=[],e(this.rows,function(b,c){
// Get the value for this editor
...
g.defaults.editors.multiple=g.AbstractEditor.extend({register:function(){if(this.editors){for(var a=0;a<this.editors.length;a++)this.editors[a]&&this.editors[a].unregister();this.editors[this.type]&&this.editors[this.type].register()}this._super()},unregister:function(){if(this._super(),this.editors)for(var a=0;a<this.editors.length;a++)this.editors[a]&&this.editors[a].unregister()},getNumColumns:function(){return this.editors[this.type]?Math.max(this.editors[this.type].getNumColumns(),4):4},enable:function(){if(!this.always_disabled){if(this.editors)for(var a=0;a<this.editors.length;a++)this.editors[a]&&this.editors[a].enable();this.switcher.disabled=!1,this._super()}},disable:function(a){if(a&&(this.always_disabled=!0),this.editors)for(var b=0;b<this.editors.length;b++)this.editors[b]&&this.editors[b].disable(a);this.switcher.disabled=!0,this._super()},switchEditor:function(a){var b=this;this.editors[a]||this.buildChildEditor(a);var c=b.getValue();b.type=a,b.register(),e(b.editors,function(a,d){d&&(b.type===a?(b.keep_values&&d.setValue(c,!0),d.container.style.display=""):d.container.style.display="none")}),b.refreshValue(),b.refreshHeaderText()},buildChildEditor:function(a){var b=this,c=this.types[a],e=b.theme.getChildEditorHolder();b.editor_holder.appendChild(e);var f;"string"==typeof c?(f=d({},b.schema),f.type=c):(f=d({},b.schema,c),f=b.jsoneditor.expandRefs(f),
// If we need to merge `required` arrays
c&&c.required&&Array.isArray(c.required)&&b.schema.required&&Array.isArray(b.schema.required)&&(f.required=b.schema.required.concat(c.required)));var g=b.jsoneditor.getEditorClass(f);b.editors[a]=b.jsoneditor.createEditor(g,{jsoneditor:b.jsoneditor,schema:f,container:e,path:b.path,parent:b,required:!0}),b.editors[a].preBuild(),b.editors[a].build(),b.editors[a].postBuild(),b.editors[a].header&&(b.editors[a].header.style.display="none"),b.editors[a].option=b.switcher_options[a],e.addEventListener("change_header_text",function(){b.refreshHeaderText()}),a!==b.type&&(e.style.display="none")},preBuild:function(){if(this.types=[],this.type=0,this.editors=[],this.validators=[],this.keep_values=!0,"undefined"!=typeof this.jsoneditor.options.keep_oneof_values&&(this.keep_values=this.jsoneditor.options.keep_oneof_values),"undefined"!=typeof this.options.keep_oneof_values&&(this.keep_values=this.options.keep_oneof_values),this.schema.oneOf)this.oneOf=!0,this.types=this.schema.oneOf,delete this.schema.oneOf;else if(this.schema.anyOf)this.anyOf=!0,this.types=this.schema.anyOf,delete this.schema.anyOf;else{if(this.schema.type&&"any"!==this.schema.type)Array.isArray(this.schema.type)?this.types=this.schema.type:this.types=[this.schema.type];else
// If any of these primitive types are disallowed
if(this.types=["string","number","integer","boolean","object","array","null"],this.schema.disallow){var a=this.schema.disallow;"object"==typeof a&&Array.isArray(a)||(a=[a]);var b=[];e(this.types,function(c,d){a.indexOf(d)===-1&&b.push(d)}),this.types=b}delete this.schema.type}this.display_text=this.getDisplayText(this.types)},build:function(){var a=this,b=this.container;this.header=this.label=this.theme.getFormInputLabel(this.getTitle()),this.container.appendChild(this.header),this.switcher=this.theme.getSwitcher(this.display_text),b.appendChild(this.switcher),this.switcher.addEventListener("change",function(b){b.preventDefault(),b.stopPropagation(),a.switchEditor(a.display_text.indexOf(this.value)),a.onChange(!0)}),this.editor_holder=document.createElement("div"),b.appendChild(this.editor_holder);var c={};a.jsoneditor.options.custom_validators&&(c.custom_validators=a.jsoneditor.options.custom_validators),this.switcher_options=this.theme.getSwitcherOptions(this.switcher),e(this.types,function(b,e){a.editors[b]=!1;var f;"string"==typeof e?(f=d({},a.schema),f.type=e):(f=d({},a.schema,e),
// If we need to merge `required` arrays
e.required&&Array.isArray(e.required)&&a.schema.required&&Array.isArray(a.schema.required)&&(f.required=a.schema.required.concat(e.required))),a.validators[b]=new g.Validator(a.jsoneditor,f,c)}),this.switchEditor(0)},onChildEditorChange:function(a){this.editors[this.type]&&(this.refreshValue(),this.refreshHeaderText()),this._super()},refreshHeaderText:function(){var a=this.getDisplayText(this.types);e(this.switcher_options,function(b,c){c.textContent=a[b]})},refreshValue:function(){this.value=this.editors[this.type].getValue()},setValue:function(a,b){
// Determine type by getting the first one that validates
var c=this,d=this.type;e(this.validators,function(b,d){if(!d.validate(a).length)return c.type=b,c.switcher.value=c.display_text[b],!1});var f=this.type!=d;f&&this.switchEditor(this.type),this.editors[this.type].setValue(a,b),this.refreshValue(),c.onChange(f)},destroy:function(){e(this.editors,function(a,b){b&&b.destroy()}),this.editor_holder&&this.editor_holder.parentNode&&this.editor_holder.parentNode.removeChild(this.editor_holder),this.switcher&&this.switcher.parentNode&&this.switcher.parentNode.removeChild(this.switcher),this._super()},showValidationErrors:function(a){var b=this;
// oneOf and anyOf error paths need to remove the oneOf[i] part before passing to child editors
if(this.oneOf||this.anyOf){var c=this.oneOf?"oneOf":"anyOf";e(this.editors,function(f,g){if(g){var h=b.path+"."+c+"["+f+"]",i=[];e(a,function(a,c){if(c.path.substr(0,h.length)===h){var e=d({},c);e.path=b.path+e.path.substr(h.length),i.push(e)}}),g.showValidationErrors(i)}})}else e(this.editors,function(b,c){c&&c.showValidationErrors(a)})}}),
...
// TODO: use theme
var c="";return e(a,function(d,e){var f=b.getHTML(e);
...
// select.js ↓
g.defaults.editors.select=g.AbstractEditor.extend({setValue:function(a,b){a=this.typecast(a||"");
// Sanitize value before setting it
...
// Enum options enumerated
if(this.schema["enum"]){var c=this.schema.options&&this.schema.options.enum_titles||[];e(this.schema["enum"],function(b,d){a.enum_options[b]=""+d,a.enum_display[b]=""+(c[b]||d),a.enum_values[b]=a.typecast(d)}),this.isRequired()||(a.enum_display.unshift(" "),a.enum_options.unshift("undefined"),a.enum_values.unshift(void 0))}else if("boolean"===this.schema.type)a.enum_display=this.schema.options&&this.schema.options.enum_titles||["true","false"],a.enum_options=["1",""],a.enum_values=[!0,!1],this.isRequired()||(a.enum_display.unshift(" "),a.enum_options.unshift("undefined"),a.enum_values.unshift(void 0));else{if(!this.schema.enumSource)throw"'select' editor requires the enum property to be set.";
...
this.initstatus(this.label),
//监听checkbox
this.checkListener()},onInputChange:function(){var a,b=this.typecast(this.input.value);
...
c.indexOf(k)!==-1?(this.input.value=k,this.value=k):(this.input.value=c[0],this.value=this.typecast(c[0]||""),this.parent?this.parent.onChildEditorChange(this):this.jsoneditor.onChange(),this.jsoneditor.notifyWatchers(this.path)),this.setupSelect2()}this._super()},enable:function(){this.always_disabled||(this.input.disabled=!1,
// 添加 ↓
this.label&&this.theme.enableLabel(this.label),"boolean"==this.schema.type?this.value=1==this.input.selectedIndex?"true":"":this.value=this.input.value,
// 添加 ↑
this.select2&&(this.select2v4?this.select2.prop("disabled",!1):this.select2.select2("enable",!0))),this.refreshValue(),this.onChange(!0),this._super()},disable:function(a){a&&(this.always_disabled=!0),this.input.disabled=!0,
// 
this.value=null,this.label&&this.theme.disableLabel(this.label),
// 
this.select2&&(this.select2v4?this.select2.prop("disabled",!0):this.select2.select2("enable",!1)),this.refreshValue(),this.onChange(!0),this._super()},destroy:function(){this.label&&this.label.parentNode&&this.label.parentNode.removeChild(this.label),this.description&&this.description.parentNode&&this.description.parentNode.removeChild(this.description),this.input&&this.input.parentNode&&this.input.parentNode.removeChild(this.input),this.select2&&(this.select2.select2("destroy"),this.select2=null),this._super()},initstatus:function(a){//初始化状态 是否禁用
this.schema.disable?(this.input.disabled=!0,this.theme.disableLabel(a),"checkbox"==this.control.firstElementChild.type&&(this.control.firstElementChild.checked=!1)):"checkbox"==this.control.firstElementChild.type&&(this.control.firstElementChild.checked=!0)},
//checkboxµã»÷¼àÌý
checkListener:function(){var a=this,b=a.control.firstElementChild;"checkbox"==b.type&&b.addEventListener("change",function(b){b.preventDefault(),b.stopPropagation(),this.checked?a.enable():a.disable()})}}),
// select.js ↑
g.defaults.editors.selectize=g.AbstractEditor.extend({setValue:function(a,b){a=this.typecast(a||"");
// Sanitize value before setting it
...
// Enum options enumerated
if(this.schema["enum"]){var c=this.schema.options&&this.schema.options.enum_titles||[];e(this.schema["enum"],function(b,d){a.enum_options[b]=""+d,a.enum_display[b]=""+(c[b]||d),a.enum_values[b]=a.typecast(d)})}else if("boolean"===this.schema.type)a.enum_display=this.schema.options&&this.schema.options.enum_titles||["true","false"],a.enum_options=["1","0"],a.enum_values=[!0,!1];else{if(!this.schema.enumSource)throw"'select' editor requires the enum property to be set.";
// Shortcut declaration for using a single array
if(this.enumSource=[],this.enum_display=[],this.enum_options=[],this.enum_values=[],Array.isArray(this.schema.enumSource))for(b=0;b<this.schema.enumSource.length;b++)
// Shorthand for watched variable
"string"==typeof this.schema.enumSource[b]?this.enumSource[b]={source:this.schema.enumSource[b]}:Array.isArray(this.schema.enumSource[b])?this.enumSource[b]=this.schema.enumSource[b]:this.enumSource[b]=d({},this.schema.enumSource[b]);else this.schema.enumValue?this.enumSource=[{source:this.schema.enumSource,value:this.schema.enumValue}]:this.enumSource=[{source:this.schema.enumSource}];
...
var a=this;if(window.jQuery&&window.jQuery.fn&&window.jQuery.fn.selectize&&(this.enum_options.length>=2||this.enum_options.length&&this.enumSource)){var b=d({},g.plugins.selectize);this.schema.options&&this.schema.options.selectize_options&&(b=d(b,this.schema.options.selectize_options)),this.selectize=window.jQuery(this.input).selectize(d(b,{
...
// Show errors for this editor
if(e(a,function(a,e){e.path===b.path?c.push(e):d.push(e)}),this.error_holder)if(c.length){this.error_holder.innerHTML="",this.error_holder.style.display="",e(c,function(a,c){b.error_holder.appendChild(b.theme.getErrorMessage(c.message))})}else this.error_holder.style.display="none"}});var h=function(){var a=document.documentElement;return a.matches?"matches":a.webkitMatchesSelector?"webkitMatchesSelector":a.mozMatchesSelector?"mozMatchesSelector":a.msMatchesSelector?"msMatchesSelector":a.oMatchesSelector?"oMatchesSelector":void 0}();
// outro.js ↓
/*
...
//  自定义布局带checkbox
getFormControlB3:function(a,b,c,d){var e=document.createElement("div"),f=d.theme.GenNonDuplicateID();if(a&&"checkbox"===b.type)e.className+=" checkbox",a.appendChild(b),e.style.marginTop="0",e.appendChild(a),b.style.position="relative",b.style.cssFloat="left";else{var g=d.theme.getCheckbox();e.className+=" form-group",a&&(g.id=f,a.setAttribute("for",f),a.className+=" control-label",a.style.lineHeight=2,a.style.cursor="pointer",a.style.userSelect="none",e.appendChild(g),e.appendChild(a)),e.appendChild(b)}return c&&e.appendChild(c),e},
//自定义布局无checkbox 无label 数组颜色专用
getFormControlB3Array:function(a,b,c,d){var e=document.createElement("div");
// group.style.marginBottom = 0;
return e.className+=" form-group",a&&(a.className+=" control-label",a.style.lineHeight=2,a.style.cursor="pointer",a.style.userSelect="none",a.style.display="none",e.appendChild(a)),e.appendChild(b),c&&e.appendChild(c),e},getIndentedPanel:function(){var a=document.createElement("div");return a.className="well well-sm",a.style.paddingBottom=0,a},
//颜色组panel
getIndentedPanelArray:function(){var a=document.createElement("div");return a.className=" arraycolor",a.style.width="28px",a.style.height="28px",a.style.marginRight="5px",a},getInfoButton:function(a){var b=document.createElement("span");b.className="glyphicon glyphicon-info-sign pull-right",b.style.padding=".25rem",b.style.position="relative",b.style.display="inline-block";var c=document.createElement("span");return c.style["font-family"]="sans-serif",c.style.visibility="hidden",c.style["background-color"]="rgba(50, 50, 50, .75)",c.style.margin="0 .25rem",c.style.color="#FAFAFA",c.style.padding=".5rem 1rem",c.style["border-radius"]=".25rem",c.style.width="25rem",c.style.transform="translateX(-27rem) translateY(-.5rem)",c.style.position="absolute",c.innerText=a,b.onmouseover=function(){c.style.visibility="visible"},b.onmouseleave=function(){c.style.visibility="hidden"},b.appendChild(c),b},getFormInputDescription:function(a){var b=document.createElement("p");return b.className="help-block",b.innerHTML=a,b},getHeaderButtonHolder:function(){var a=this.getButtonHolder();return a.style.marginLeft="10px",a},getButtonHolder:function(){var a=document.createElement("div");return a.className="btn-group",a},getButton:function(a,b,c){var d=this._super(a,b,c);return d.className+="btn btn-default",d.style.backgroundColor="transparent",d.style.border="none",d},getTable:function(){var a=document.createElement("table");return a.className="table table-bordered",a.style.width="auto",a.style.maxWidth="none",a},addInputError:function(a,b){return a.controlgroup?(a.controlgroup.className+=" has-error",a.errmsg?a.errmsg.style.display="":(a.errmsg=document.createElement("p"),a.errmsg.className="help-block errormsg",a.controlgroup.appendChild(a.errmsg)),void(a.errmsg.textContent=b)):void(this.queuedInputErrorText=b)},removeInputError:function(a){a.controlgroup||delete this.queuedInputErrorText,a.errmsg&&(a.errmsg.style.display="none",a.controlgroup.className=a.controlgroup.className.replace(/\s?has-error/g,""))},getTabHolder:function(){var a=document.createElement("div");return a.innerHTML="<div class='tabs list-group col-md-2'></div><div class='col-md-10'></div>",a.className="rows",a},getHTabHolder:function(){var a=document.createElement("div");return a.innerHTML="<div class='tabs list-group col-md-12 clearpad'></div><div class='col-md-12 well well-sm clearpad'></div>",a.className="rows",a},getTab:function(a){var b=document.createElement("a");return b.className="list-group-item",b.setAttribute("href","#"),b.appendChild(a),b},getHTab:function(a){var b=document.createElement("a");return b.className="list-group-item col-md-6",b.setAttribute("href","#"),b.appendChild(a),b},markTabActive:function(a){a.className+=" active"},markTabInactive:function(a){a.className=a.className.replace(/\s?active/g,"")},getProgressBar:function(){var a=0,b=100,c=0,d=document.createElement("div");d.className="progress";var e=document.createElement("div");return e.className="progress-bar",e.setAttribute("role","progressbar"),e.setAttribute("aria-valuenow",c),e.setAttribute("aria-valuemin",a),e.setAttribute("aria-valuenax",b),e.innerHTML=c+"%",d.appendChild(e),d},updateProgressBar:function(a,b){if(a){var c=a.firstChild,d=b+"%";c.setAttribute("aria-valuenow",b),c.style.width=d,c.innerHTML=d}},updateProgressBarUnknown:function(a){if(a){var b=a.firstChild;a.className="progress progress-striped active",b.removeAttribute("aria-valuenow"),b.style.width="100%",b.innerHTML=""}}}),g.defaults.themes.bootstrap4=g.AbstractTheme.extend({getSelectInput:function(a){var b=this._super(a);
//el.style.width = 'auto';
...
return this._super(a,b,c)},getFormInputField:function(a){var b=this._super(a);return"checkbox"!==a&&(b.className+="form-control"),b},getFormControl:function(a,b,c){var d=document.createElement("div");return a&&"checkbox"===b.type?(d.className+=" checkbox",a.appendChild(b),a.style.fontSize="14px",d.style.marginTop="0",d.appendChild(a),b.style.position="relative",b.style.cssFloat="left"):(d.className+=" form-group",a&&(a.className+=" form-control-label",d.appendChild(a)),d.appendChild(b)),c&&d.appendChild(c),d},getIndentedPanel:function(){var a=document.createElement("div");return a.className="card card-block bg-faded",a.style.paddingBottom=0,a},getFormInputDescription:function(a){var b=document.createElement("p");return b.className="form-text",b.innerHTML=a,b},getHeaderButtonHolder:function(){var a=this.getButtonHolder();return a.style.marginLeft="10px",a},getButtonHolder:function(){var a=document.createElement("div");return a.className="btn-group",a},getButton:function(a,b,c){var d=this._super(a,b,c);return d.className+="btn btn-secondary",d},getTable:function(){var a=document.createElement("table");return a.className="table-bordered table-sm",a.style.width="auto",a.style.maxWidth="none",a},addInputError:function(a,b){a.controlgroup&&(a.controlgroup.className+=" has-error",a.errmsg?a.errmsg.style.display="":(a.errmsg=document.createElement("p"),a.errmsg.className="form-text errormsg",a.controlgroup.appendChild(a.errmsg)),a.errmsg.textContent=b)},removeInputError:function(a){a.errmsg&&(a.errmsg.style.display="none",a.controlgroup.className=a.controlgroup.className.replace(/\s?has-error/g,""))},getTabHolder:function(){var a=document.createElement("div");return a.innerHTML="<div class='tabs list-group col-md-2'></div><div class='col-md-10'></div>",a.className="rows",a},getTab:function(a){var b=document.createElement("a");return b.className="list-group-item-action",b.setAttribute("href","#"),b.appendChild(a),b},markTabActive:function(a){a.className+=" active"},markTabInactive:function(a){a.className=a.className.replace(/\s?active/g,"")},getProgressBar:function(){var a=0,b=100,c=0,d=document.createElement("div");d.className="progress";var e=document.createElement("div");return e.className="progress-bar",e.setAttribute("role","progressbar"),e.setAttribute("aria-valuenow",c),e.setAttribute("aria-valuemin",a),e.setAttribute("aria-valuenax",b),e.innerHTML=c+"%",d.appendChild(e),d},updateProgressBar:function(a,b){if(a){var c=a.firstChild,d=b+"%";c.setAttribute("aria-valuenow",b),c.style.width=d,c.innerHTML=d}},updateProgressBarUnknown:function(a){if(a){var b=a.firstChild;a.className="progress progress-striped active",b.removeAttribute("aria-valuenow"),b.style.width="100%",b.innerHTML=""}}}),g.defaults.themes.foundation=g.AbstractTheme.extend({getChildEditorHolder:function(){var a=document.createElement("div");return a.style.marginBottom="15px",a},getSelectInput:function(a){var b=this._super(a);return b.style.minWidth="none",b.style.padding="5px",b.style.marginTop="3px",b},getSwitcher:function(a){var b=this._super(a);return b.style.paddingRight="8px",b},afterInputReady:function(a){if(!a.group&&(this.closest(a,".compact")&&(a.style.marginBottom=0),a.group=this.closest(a,".form-control"),this.queuedInputErrorText)){var b=this.queuedInputErrorText;delete this.queuedInputErrorText,this.addInputError(a,b)}},getFormInputLabel:function(a){var b=this._super(a);return b.style.display="inline-block",b},getFormInputField:function(a){var b=this._super(a);return b.style.width="100%",b.style.marginBottom="checkbox"===a?"0":"12px",b},getFormInputDescription:function(a){var b=document.createElement("p");return b.textContent=a,b.style.marginTop="-10px",b.style.fontStyle="italic",b},getIndentedPanel:function(){var a=document.createElement("div");return a.className="panel",a.style.paddingBottom=0,a},getHeaderButtonHolder:function(){var a=this.getButtonHolder();return a.style.display="inline-block",a.style.marginLeft="10px",a.style.verticalAlign="middle",a},getButtonHolder:function(){var a=document.createElement("div");return a.className="button-group",a},getButton:function(a,b,c){var d=this._super(a,b,c);return d.className+=" small button",d},addInputError:function(a,b){return a.group?(a.group.className+=" error",a.errmsg?a.errmsg.style.display="":(a.insertAdjacentHTML("afterend",'<small class="error"></small>'),a.errmsg=a.parentNode.getElementsByClassName("error")[0]),void(a.errmsg.textContent=b)):void(this.queuedInputErrorText=b)},removeInputError:function(a){a.group||delete this.queuedInputErrorText,a.errmsg&&(a.group.className=a.group.className.replace(/ error/g,""),a.errmsg.style.display="none")},getProgressBar:function(){var a=document.createElement("div");a.className="progress";var b=document.createElement("span");return b.className="meter",b.style.width="0%",a.appendChild(b),a},updateProgressBar:function(a,b){a&&(a.firstChild.style.width=b+"%")},updateProgressBarUnknown:function(a){a&&(a.firstChild.style.width="100%")}}),g.defaults.themes.foundation3=g.defaults.themes.foundation.extend({getHeaderButtonHolder:function(){var a=this._super();return a.style.fontSize=".6em",a},getFormInputLabel:function(a){var b=this._super(a);return b.style.fontWeight="bold",b},getTabHolder:function(){var a=document.createElement("div");return a.className="row",a.innerHTML="<dl class='tabs vertical two columns'></dl><div class='tabs-content ten columns'></div>",a},setGridColumnSize:function(a,b){var c=["zero","one","two","three","four","five","six","seven","eight","nine","ten","eleven","twelve"];a.className="columns "+c[b]},getTab:function(a){var b=document.createElement("dd"),c=document.createElement("a");return c.setAttribute("href","#"),c.appendChild(a),b.appendChild(c),b},getTabContentHolder:function(a){return a.children[1]},getTabContent:function(){var a=document.createElement("div");return a.className="content active",a.style.paddingLeft="5px",a},markTabActive:function(a){a.className+=" active"},markTabInactive:function(a){a.className=a.className.replace(/\s*active/g,"")},addTab:function(a,b){a.children[0].appendChild(b)}}),g.defaults.themes.foundation4=g.defaults.themes.foundation.extend({getHeaderButtonHolder:function(){var a=this._super();return a.style.fontSize=".6em",a},setGridColumnSize:function(a,b){a.className="columns large-"+b},getFormInputDescription:function(a){var b=this._super(a);return b.style.fontSize=".8rem",b},getFormInputLabel:function(a){var b=this._super(a);return b.style.fontWeight="bold",b}}),g.defaults.themes.foundation5=g.defaults.themes.foundation.extend({getFormInputDescription:function(a){var b=this._super(a);return b.style.fontSize=".8rem",b},setGridColumnSize:function(a,b){a.className="columns medium-"+b},getButton:function(a,b,c){var d=this._super(a,b,c);return d.className=d.className.replace(/\s*small/g,"")+" tiny",d},getTabHolder:function(){var a=document.createElement("div");return a.innerHTML="<dl class='tabs vertical'></dl><div class='tabs-content vertical'></div>",a},getTab:function(a){var b=document.createElement("dd"),c=document.createElement("a");return c.setAttribute("href","#"),c.appendChild(a),b.appendChild(c),b},getTabContentHolder:function(a){return a.children[1]},getTabContent:function(){var a=document.createElement("div");return a.className="content active",a.style.paddingLeft="5px",a},markTabActive:function(a){a.className+=" active"},markTabInactive:function(a){a.className=a.className.replace(/\s*active/g,"")},addTab:function(a,b){a.children[0].appendChild(b)}}),g.defaults.themes.foundation6=g.defaults.themes.foundation5.extend({getIndentedPanel:function(){var a=document.createElement("div");return a.className="callout secondary",a},getButtonHolder:function(){var a=document.createElement("div");return a.className="button-group tiny",a.style.marginBottom=0,a},getFormInputLabel:function(a){var b=this._super(a);return b.style.display="block",b},getFormControl:function(a,b,c,d){var e=document.createElement("div");return e.className="form-control",a&&e.appendChild(a),"checkbox"===b.type?a.insertBefore(b,a.firstChild):a?(d&&a.appendChild(d),a.appendChild(b)):(d&&e.appendChild(d),e.appendChild(b)),c&&a.appendChild(c),e},addInputError:function(a,b){if(a.group){if(a.group.className+=" error",a.errmsg)a.errmsg.style.display="",a.className="";else{var c=document.createElement("span");c.className="form-error is-visible",a.group.getElementsByTagName("label")[0].appendChild(c),a.className=a.className+" is-invalid-input",a.errmsg=c}a.errmsg.textContent=b}},removeInputError:function(a){a.errmsg&&(a.className=a.className.replace(/ is-invalid-input/g,""),a.errmsg.parentNode&&a.errmsg.parentNode.removeChild(a.errmsg))}}),g.defaults.themes.html=g.AbstractTheme.extend({/*
  ...
  */
getFormInputDescription:function(a){var b=this._super(a);return b.style.fontSize=".8em",b.style.margin=0,b.style.display="inline-block",b.style.fontStyle="italic",b},/*
  ...
  */
getChildEditorHolder:function(){var a=this._super();return a.style.marginBottom="8px",a},getHeaderButtonHolder:function(){var a=this.getButtonHolder();return a.style.display="inline-block",a.style.marginLeft="10px",a.style.fontSize=".8em",a.style.verticalAlign="middle",a},getTable:function(){var a=this._super();return a.style.borderBottom="1px solid #ccc",a.style.marginBottom="5px",a},addInputError:function(a,b){if(a.style.borderColor="red",a.errmsg)a.errmsg.style.display="block";else{var c=this.closest(a,".form-control");a.errmsg=document.createElement("div"),a.errmsg.setAttribute("class","errmsg"),a.errmsg.style=a.errmsg.style||{},a.errmsg.style.color="red",c.appendChild(a.errmsg)}a.errmsg.innerHTML="",a.errmsg.appendChild(document.createTextNode(b))},removeInputError:function(a){a.style.borderColor="",a.errmsg&&(a.errmsg.style.display="none")},getProgressBar:function(){var a=100,b=0,c=document.createElement("progress");return c.setAttribute("max",a),c.setAttribute("value",b),c},updateProgressBar:function(a,b){a&&a.setAttribute("value",b)},updateProgressBarUnknown:function(a){a&&a.removeAttribute("value")},getSwitchFormControl:function(a,b,c,d){var e=document.createElement("div"),f=document.createElement("div"),g=document.createElement("label");e.className="form-group",f.className="switch",a&&e.appendChild(a);var h=this.GenNonDuplicateID();return b.id=h,g.setAttribute("for",h),g.style.userSelect="none",b.style.display="none",d.schema["default"]?b.setAttribute("data-switch","on"):b.setAttribute("data-switch","off"),f.appendChild(b),f.appendChild(g),e.appendChild(f),c&&e.appendChild(c),e},getFormControlB3:function(a,b,c){var d=document.createElement("div");return a&&"checkbox"===b.type?(d.className+=" checkbox",a.appendChild(b),a.style.fontSize="14px",d.style.marginTop="0",d.appendChild(a),b.style.position="relative",b.style.cssFloat="left"):(d.className+=" form-group",a&&(a.className+=" control-label",d.appendChild(a)),d.appendChild(b)),c&&d.appendChild(c),d},getFormControlB3CK:function(a,b,c,d){var e=document.createElement("div"),f=d.theme.GenNonDuplicateID();if(a&&"checkbox"===b.type)e.className+=" checkbox",a.appendChild(b),e.style.marginTop="0",e.appendChild(a),b.style.position="relative",b.style.cssFloat="left";else{var g=d.theme.getCheckbox();e.className+=" form-group",a&&(g.id=f,a.setAttribute("for",f),a.className+=" control-label",a.style.lineHeight=2,a.style.cursor="pointer",a.style.userSelect="none",e.appendChild(g),e.appendChild(a)),e.appendChild(b)}return c&&e.appendChild(c),e},getIndentedPanel:function(){var a=document.createElement("div");return a.className="v3-base-json-editor__well",a.style.paddingBottom=0,a},
//颜色组panel
getIndentedPanelArray:function(){var a=document.createElement("div");return a.className=" arraycolor",a.style.width="28px",a.style.height="28px",a.style.marginRight="5px",a},
//自定义布局无checkbox 无label 数组颜色专用
getFormControlB3Array:function(a,b,c,d){var e=document.createElement("div");
// group.style.marginBottom = 0;
return e.className+=" form-group",a&&(a.className+=" control-label",a.style.lineHeight=2,a.style.cursor="pointer",a.style.userSelect="none",a.style.display="none",e.appendChild(a)),e.appendChild(b),c&&e.appendChild(c),e},getHTabHolder:function(){var a=document.createElement("div");return a.innerHTML="<div class='tabs list-group col-md-12 clearpad'></div><div class='col-md-12 well well-sm clearpad'></div>",a.className="rows",a},getHTab:function(a){var b=document.createElement("a");return b.className="list-group-item col-md-6",b.setAttribute("href","#"),b.appendChild(a),b},setGridColumnSize:function(a,b){a.className="col-md-"+b},getSelectInput:function(a){var b=this._super(a);
//el.style.width = 'auto';
return b.className+="form-control",b},getTextareaInput:function(){var a=document.createElement("textarea");return a.className="form-control",a},getFormInputField:function(a){var b=this._super(a);return"checkbox"!==a&&(b.className+="form-control"),b},afterInputReady:function(a){a.controlgroup||(a.controlgroup=this.closest(a,".form-group"),this.closest(a,".compact")&&(a.controlgroup.style.marginBottom=0))},getButton:function(a,b,c){var d=this._super(a,b,c);return d.className+="btn btn-default",d.style.backgroundColor="transparent",d.style.border="none",d}}),g.defaults.themes.jqueryui=g.AbstractTheme.extend({getTable:function(){var a=this._super();return a.setAttribute("cellpadding",5),a.setAttribute("cellspacing",0),a},getTableHeaderCell:function(a){var b=this._super(a);return b.className="ui-state-active",b.style.fontWeight="bold",b},getTableCell:function(){var a=this._super();return a.className="ui-widget-content",a},getHeaderButtonHolder:function(){var a=this.getButtonHolder();return a.style.marginLeft="10px",a.style.fontSize=".6em",a.style.display="inline-block",a},getFormInputDescription:function(a){var b=this.getDescription(a);return b.style.marginLeft="10px",b.style.display="inline-block",b},getFormControl:function(a,b,c,d){var e=this._super(a,b,c,d);return"checkbox"===b.type?(e.style.lineHeight="25px",e.style.padding="3px 0"):e.style.padding="4px 0 8px 0",e},getDescription:function(a){var b=document.createElement("span");return b.style.fontSize=".8em",b.style.fontStyle="italic",b.textContent=a,b},getButtonHolder:function(){var a=document.createElement("div");return a.className="ui-buttonset",a.style.fontSize=".7em",a},getFormInputLabel:function(a){var b=document.createElement("label");return b.style.fontWeight="bold",b.style.display="block",b.textContent=a,b},getButton:function(a,b,c){var d=document.createElement("button");d.className="ui-button ui-widget ui-state-default ui-corner-all",
// Icon only
...
button_collapse:"Collapse",/**
    ...
    */
button_expand:"Expand"},g.plugins={ace:{theme:""},SimpleMDE:{},sceditor:{},select2:{},selectize:{},colorpicker:{},checkboxradio:{},minicolor:{animationSpeed:50,animationEasing:"swing",change:null,changeDelay:0,control:"hux",defaultValue:"#ff6161",format:"hex",hide:null,hideSpeed:300,inline:!1,keywords:"transparent",letterCase:"lowercase",opacity:!1,position:"bottom left",show:null,showSpeed:300,theme:"bootstrap",swatches:["#ef9a9a","#90caf9","#a5d6a7","#fff59d","#ffcc80","#bcaaa4","transparent"]}},e(g.defaults.editors,function(a,b){g.defaults.editors[a].options=b.options||{}}),g.defaults.resolvers.unshift(function(a){if("string"!=typeof a.type)return"multiple"}),g.defaults.resolvers.unshift(function(a){
// If the schema is a simple type
if(!a.type&&a.properties)return"object"}),g.defaults.resolvers.unshift(function(a){
// If the schema is a simple type
if("string"==typeof a.type)return a.type}),g.defaults.resolvers.unshift(function(a){if("integer"===a.type&&"rating"===a.format)return"rating"}),g.defaults.resolvers.unshift(function(a){if("boolean"===a.type)
// If explicitly set to 'checkbox', use that
// if(schema.format === "checkbox" || (schema.options && schema.options.checkbox)) {
// If explicitly set to 'checkbox', use that
// if(schema.format === "checkbox" || (schema.options && schema.options.checkbox)) {
return"checkbox"===a.format||"switch"===a.format||a.options&&a.options.checkbox?"checkbox":g.plugins.selectize.enable?"selectize":"select"}),g.defaults.resolvers.unshift(function(a){
// If the schema can be of any type
if("any"===a.type)return"multiple"}),g.defaults.resolvers.unshift(function(a){
// If the schema can be of any type
if("string"===a.type&&a.media&&"base64"===a.media.binaryEncoding)return"base64"}),g.defaults.resolvers.unshift(function(a){if("string"===a.type&&"url"===a.format&&a.options&&a.options.upload===!0&&window.FileReader)return"upload"}),g.defaults.resolvers.unshift(function(a){
// Type `array` with format set to `table`
if("array"===a.type&&"table"===a.format)return"table"}),g.defaults.resolvers.unshift(function(a){if("integer"===a.type&&"rating"===a.format)return"rating"}),g.defaults.resolvers.unshift(function(a){if("string"===a.type&&"colorpicker"===a.format)return"colorpicker"}),g.defaults.resolvers.unshift(function(a){if("string"===a.type&&"checkboxradio"===a.format)return"checkboxradio"}),g.defaults.resolvers.unshift(function(a){if("string"===a.type&&"jpicker"===a.format)return"jpicker"}),g.defaults.resolvers.unshift(function(a){if("string"===a.type&&"minicolor"===a.format)return"minicolor"}),g.defaults.resolvers.unshift(function(a){if("string"===a.type&&"acedit"===a.format)return"acedit"}),g.defaults.resolvers.unshift(function(a){if(a.enumSource)return g.plugins.selectize.enable?"selectize":"select"}),g.defaults.resolvers.unshift(function(a){if(a["enum"]){if("array"===a.type||"object"===a.type)return"enum";if("number"===a.type||"integer"===a.type||"string"===a.type)return g.plugins.selectize.enable?"selectize":"select"}}),g.defaults.resolvers.unshift(function(a){if("array"===a.type&&a.items&&!Array.isArray(a.items)&&a.uniqueItems&&["string","number","integer"].indexOf(a.items.type)>=0){
// For enumerated strings, number, or integers
if(a.items["enum"])return"multiselect";if(g.plugins.selectize.enable&&"string"===a.items.type)return"arraySelectize"}}),g.defaults.resolvers.unshift(function(a){
// If this schema uses `oneOf` or `anyOf`
if(a.oneOf||a.anyOf)return"multiple"}),function(){if(window.jQuery||window.Zepto){var a=window.jQuery||window.Zepto;a.jsoneditor=g.defaults,a.fn.jsoneditor=function(a){var b=this,c=this.data("jsoneditor");if("value"===a){if(!c)throw"Must initialize jsoneditor before getting/setting the value";
// Set value
...
c&&c.destroy(),
// Create editor
c=new g(this.get(0),a),this.data("jsoneditor",c),
// Setup event listeners
c.on("change",function(){b.trigger("change")}),c.on("ready",function(){b.trigger("ready")}))}return this}}}(),g});
